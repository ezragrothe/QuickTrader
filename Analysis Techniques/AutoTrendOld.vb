...
'                            If TypeOf a.AnalysisTechnique Is AutoTrendBase Then
'                                CType(a.AnalysisTechnique, AutoTrendBase).ApplyRV(value)
'                            End If
'						Next
'					End If
'					Chart.ReApplyAnalysisTechnique(Me)
...
'		<Input("The RV text size.")>
'		Public Property RVTextSize As Decimal = 12
...
'		<Input("")> Property SwingCountTextLocation As Point = New Point(200, 50)
'		<Input("")> Property SwingCountTextColor As Color = Colors.Black
...
'		<Input("The font weight for pivot and length texts.")>
'		Public Property LengthTextFontWeight As FontWeight = FontWeights.Normal
...
'        <Input("Makes the outer channel lines transparent.")>
'        Public Property MakeOuterLinesTransparent As Boolean = True
'        <Input("The visibilty for potential channel lines.")>
'        Public Property PotentialChannelLinesVisible As Boolean = False
...
'		<Input("The color for potential up gap lines.")>
'		Public Property PotentialUpGapChannelLineColor As Color = Colors.Pink
'		<Input("The color for potential down gap lines.")>
'		Public Property PotentialDownGapChannelLineColor As Color = Colors.Pink
...
'        <Input("The color for the potential swing channels.")>
'        Public Property SwingChannelPotentialColor As Color = Colors.Gray
'        <Input("The color for the swing channels.")>
'        Public Property SwingChannelColor As Color = Colors.Gray
'        <Input("The color for the middle swing channel lines.")>
'        Public Property SwingChannelMiddleColor As Color = Colors.Orange
'        <Input("The color for the middle swing potential channel lines.")>
'        Public Property SwingChannelMiddlePotentialColor As Color = Colors.Gray

'        '<Input("The color for the confirmed gapped swing channels.")>
'        'Public Property SwingChannelConfirmedGappedColor As Color = Colors.Magenta
'        '<Input("The line width for the potential gapped swing channels.")>
'        'Public Property SwingChannelConfirmedGappedLineWidth As Decimal = 1
...
'        '<Input("The color for the potential swing channels.")>
'        'Public Property SwingChannelPotentialColor As Color = Colors.LightGray
'        <Input("The option to remove potential swing channels.")>
'        Public Property RemoveSwingChannelPotentials As Boolean = False
...
'		<Input("The key to enable syncing of the RV pad for charts in a parent workspace.")>
...
'		<Input("")>
'		Public Property UseRegressionConfirmedSwingChannels As Boolean = True
'		<Input("")>
'		Public Property UseRegressionPotentialSwingChannels As Boolean = True
...
'			End Get
'		End Property
'		Private ReadOnly Property CurrentChannelLineIfPotential(hidden As Boolean) As Channel
'			Get
'				If channels.Count > 0 AndAlso Not channels(channels.Count - 1).IsConfirmed AndAlso channels(channels.Count - 1).Hidden = hidden Then
'					Return channels(channels.Count - 1)
'				ElseIf channels.Count > 1 AndAlso Not channels(channels.Count - 2).IsConfirmed AndAlso channels(channels.Count - 2).Hidden = hidden Then
'					Return channels(channels.Count - 2)
'				Else
'					Return Nothing
'				End If
'				Return Nothing
...
'			MyBase.Begin()
'			For Each c In Chart.SlaveCharts
'				For Each tech In c.AnalysisTechniques
'					If TypeOf tech.AnalysisTechnique Is SwingOverlay Then
'						If CType(tech.AnalysisTechnique, SwingOverlay).lines.Count > 0 Then
'							Dim chart As Chart = CType(tech.AnalysisTechnique, SwingOverlay).lines(0).Parent
'							For Each l In CType(tech.AnalysisTechnique, SwingOverlay).lines
'								l.Children.Remove(l)
'							Next
'							chart.ReApplyAnalysisTechnique(CType(tech.AnalysisTechnique, SwingOverlay))
'						End If
'					End If
'				Next
'			Next

'			timeLabel = NewLabel(Now.ToShortTimeString, SwingCountTextColor, Chart.GetRelativeFromReal(SwingCountTextLocation))
...
'			currentSwingLengthText.Font.FontWeight = LengthTextFontWeight
...
'			rvText.Font.FontSize = RVTextSize
'			rvText.Font.FontWeight = FontWeights.Bold
...
'			currentSwingPotentialBCTargetText = NewLabel("", SwingTextColor, New Point(0, 0), True)
...
'			currentSwingPotentialBCTargetText.Font.FontSize = LengthTextFontSize
'			currentSwingPotentialBCTargetText.Font.FontWeight = LengthTextFontWeight
'			currentSwingPotentialBCTargetText.Font.Brush = New SolidColorBrush(SwingChannelBCLengthTextColor)
'            swingChannel = New SwingChannelLine(NewTrendLine(SwingChannelPotentialColor, New Point(0, 0), New Point(0, 0), Not RemoveSwingChannelPotentials))
...
'			End If
'            currentSwingCountGraphLine = NewTrendLine(SwingChannelColor, New Point(CurrentBar.Number, swingCountGraphYPlacement), New Point(CurrentBar.Number, swingCountGraphYPlacement))
...
'					For Each tech In c.AnalysisTechniques
'						If TypeOf tech.AnalysisTechnique Is SwingOverlay Then
'							CType(tech.AnalysisTechnique, SwingOverlay).AddLine(Chart.bars(cs.EndBar).Data.Date, CurrentBar.Date, Direction.Up)
'						End If
'					Next
'				Next
...
'					For Each tech In c.AnalysisTechniques
'						If TypeOf tech.AnalysisTechnique Is SwingOverlay Then
'							CType(tech.AnalysisTechnique, SwingOverlay).AddLine(Chart.bars(cs.EndBar).Data.Date, CurrentBar.Date, Direction.Down)
'						End If
'					Next
'				Next
'				If swings.Count > 0 And SwingLinesVisible Then
...
'					For Each tech In c.AnalysisTechniques
'						If TypeOf tech.AnalysisTechnique Is SwingOverlay Then
'							CType(tech.AnalysisTechnique, SwingOverlay).ExtendLine(CurrentBar.Date, Direction.Up)
'						End If
'					Next
'				Next
...
'					For Each tech In c.AnalysisTechniques
'						If TypeOf tech.AnalysisTechnique Is SwingOverlay Then
'							CType(tech.AnalysisTechnique, SwingOverlay).ExtendLine(CurrentBar.Date, Direction.Down)
'						End If
'					Next
'				Next
'				If swings.Count > 0 Then
...
'					If swings.Count > 1 Then
'						'Log("swing event detected at bar " & CurrentBar.Number)
...
'								'Log("bgap line drawn at bar " & CurrentBar.Number)
...
'							'Log("gapline destroyed at bar " & CurrentBar.Number)
'						End If
'					End If
'				End If
'			End If
...
'            If swings.Count > 2 AndAlso curSwingEvent <> AutoTrendBase.SwingEvent.None Then ' AndAlso (channelLines.Count = 0 OrElse swingDir <> CurrentChannelLine.Direction) Then
...
'                    '        If CurrentSwing.EndPrice > swings(swings.Count - 2).StartPrice And CurrentSwing.StartPrice > swings(swings.Count - 3).StartPrice Then
'                    '            BarColor(swings(swings.Count - 3).StartBar, CurrentBar.Number, UpTrendBarColor, UpTrendSwingLineColor, Swing.ChannelDirectionType.Down)
'                    '        End If
'                    '    ElseIf swingDir = Direction.Down Then
'                    '        If CurrentSwing.EndPrice < swings(swings.Count - 2).StartPrice And CurrentSwing.StartPrice < swings(swings.Count - 3).StartPrice Then
'                    '            BarColor(swings(swings.Count - 3).StartBar, CurrentBar.Number, DownTrendBarColor, DownTrendSwingLineColor, Swing.ChannelDirectionType.Down)
'                    '        End If
'                    '    End If
'                    'End If
'                    'If swings(lastConfirmedSwingIndex).ChannelDirection = Swing.ChannelDirectionType.Down And CurrentSwing IsNot swings(lastConfirmedSwingIndex) And CurrentSwing.Direction = swings(lastConfirmedSwingIndex).Direction And CurrentSwing.EndPrice < swings(lastConfirmedSwingIndex).EndPrice Then
'                    '    BarColor(swings(lastConfirmedSwingIndex).EndBar, CurrentBar.Number, DownTrendBarColor, DownTrendSwingLineColor, Swing.ChannelDirectionType.Down)
'                    'ElseIf swings(lastConfirmedSwingIndex).ChannelDirection = Swing.ChannelDirectionType.Up And CurrentSwing IsNot swings(lastConfirmedSwingIndex) And CurrentSwing.Direction = swings(lastConfirmedSwingIndex).Direction And CurrentSwing.EndPrice > swings(lastConfirmedSwingIndex).EndPrice Then
'                    '    BarColor(swings(lastConfirmedSwingIndex).EndBar, CurrentBar.Number, UpTrendBarColor, UpTrendSwingLineColor, Swing.ChannelDirectionType.Down)
'                    'End If
'                End If
'                'If ChannelMode <> ChannelModeType.Merged Then
'                'If ChannelMode <> ChannelModeType.Merged Then
...
'                    If channels(k).IsTrendModeChannel And channels(k).APoint.X < swings(firstConfirmedSwingIndex).StartBar And channels(k).CPoint.X > swings(firstConfirmedSwingIndex).StartBar Then
...
'                        RemoveObjectFromChart(channels(k).MiddleTL)
...
'                    DrawChannelIfLegalPlacement(aPoint, bPoint, cPoint, setPoints, False, True, True)
'                End If
'                'End If
...
'                DrawChannelIfLegalPlacement(aPoint, bPoint, cPoint, setPoints, ChannelMode <> ChannelModeType.Merged, False, False)
'                'End If
...
'                            RemoveObjectFromChart(channel.TL)
'                            RemoveObjectFromChart(channel.MiddleTL)
...
'                                    If channel.CutBarNumber <> -1 And channel.CutBarNumber < CurrentSwing.StartBar Then channel.CutBarNumber = -1
'                                End If
'                            Else
...
'                                channel.TL.Pen = New Pen(New SolidColorBrush(If(MakeOuterLinesTransparent = False, If(channel.Direction = Direction.Down, ConfirmedDownChannelLineColor, ConfirmedUpChannelLineColor), Colors.Transparent)), 1)
'                                channel.MiddleTL.Pen = New Pen(New SolidColorBrush(If(channel.Direction = Direction.Down, ConfirmedDownChannelLineColor, ConfirmedUpChannelLineColor)), 1)
...
'                                End If
'                                If Not channel.Hidden And channel.DontDraw = False And channel.BCLengthText IsNot Nothing Then
...
'                                If ShowOnlyLastConfirmedChannels = False Then channel.IsBackedUp = False
...
'                            'channel.TL.Pen = New Pen(New SolidColorBrush(If(channel.Direction = Directi1on.Down, ConfirmedHistoryDownChannelLineColor, ConfirmedHistoryUpChannelLineColor)), 1)
...
'                            If RV < HistoryBCLengthVisibilityRVCutoff * If(CustomRangeValue = -1, Chart.Settings("RangeValue").Value, CustomRangeValue) Then RemoveObjectFromChart(channel.BCLengthText)
...
'                        If channel.IsCancelled = False And swingDir <> channel.Direction And channel.CPoint.X <> cPoint.X Then
...
'                        '	channel.TL.Pen.Brush = New SolidColorBrush(If(channel.Direction = Direction.Up, PotentialUpGapChannelLineColor, PotentialDownGapChannelLineColor))
'                        '	If channel.IsBackedUp And channel.Hidden = False Then
'                        '		channel.IsBackedUp = False
'                        '		If ChannelLinesVisible Then AddObjectToChart(channel.TL)
'                        '	End If
'                        'End If
...
'                        '	End If
'                        'End If
'                        If channel.IsConfirmed = True And channel.IsCut = False And swingDir = channel.Direction And UseRegressionConfirmedChannels Then
...
'                                Dim middleLine As LineCoordinates = CalculateNewStartPointRegressionMiddle(channel.TL.StartPoint.X, channel.APoint, channel.CPoint)
'                                Dim cdif As Decimal = channel.CPoint.Y - middleLine.EndPoint.Y
'                                Dim adif As Decimal = channel.APoint.Y - middleLine.StartPoint.Y
'                                Dim dif As Decimal = adif - cdif
'                                channel.TL.StartPoint = New Point(channel.TL.StartPoint.X, channel.APoint.Y - dif)
'                                channel.TL.ParallelDistance = channel.BPoint.Y - LinCalc(channel.TL.StartPoint.X, channel.TL.StartPoint.Y, channel.TL.EndPoint.X, channel.TL.EndPoint.Y, channel.BPoint.X)
'                                channel.MiddleTL.StartPoint = middleLine.StartPoint
'                                channel.MiddleTL.EndPoint = New Point(channel.TL.EndPoint.X, LinCalc(middleLine.StartPoint.X, middleLine.StartPoint.Y, middleLine.EndPoint.X, middleLine.EndPoint.Y, channel.TL.EndPoint.X))
'                            End If
'                        End If
...
'                If channel.IsConfirmed = False And channel.IsCancelled = False And UseRegressionPotentialChannels Then
'                    Dim middleLine As LineCoordinates = CalculateNewStartPointRegressionMiddle(channel.TL.StartPoint.X, channel.APoint, channel.CPoint)
'                    Dim cdif As Decimal = channel.CPoint.Y - middleLine.EndPoint.Y
'                    Dim adif As Decimal = channel.APoint.Y - middleLine.StartPoint.Y
'                    Dim dif As Decimal = adif - cdif
...
'                sumx2 += point.X ^ 2
'            Next
'            If (n * sumx2 - sumx * sumx) = 0 Then b = 0 Else b = (n * sumxy - sumx * sumy) / (n * sumx2 - sumx * sumx)
'            a = (sumy - b * sumx) / n
...
'                         ) And
'                          (
'                              (channel.IsConfirmed And
'                                    (
'                                        (ShowOnlyLastConfirmedChannels And channel.IsBackedUp = False) Or
'                                         ShowOnlyLastConfirmedChannels = False
'                                    )
'                                ) Or
'                                (channel.IsConfirmed = False And
'                                    (
'                                        (ShowOnlyLastPotentialChannels And channel.IsBackedUp = False) Or
'                                         ShowOnlyLastPotentialChannels = False
'                                    )
'                                )
'                            ) And
'                            (
'                                channel.Hidden = False And channel.DontDraw = False And ChannelLinesVisible
'                            ) Then
'                            If (PotentialChannelLinesVisible And channel.IsConfirmed = False) Or channel.IsConfirmed Then
'                                AddObjectToChart(channel.TL)
'                                channel.TL.RefreshVisual()
'                                AddObjectToChart(channel.MiddleTL)
'                                'Dim zz = channel.MiddleTL
'                                channel.MiddleTL.RefreshVisual()
'                            End If
'                        End If
...
'                        '    If channel.Hidden = False And channel.DontDraw = False And ChannelLinesVisible Then
'                        '        If (PotentialChannelLinesVisible And channel.IsConfirmed = False) Or channel.IsConfirmed Then
'                        '            AddObjectToChart(channel.TL)
'                        '        End If
'                        '    End If
...
'		End Function
'		Private Function GetChannelLineFromHidden(hidden As Boolean, numberBack As Integer) As Channel
'			Dim index As Integer = 0
'			For i = channels.Count - 1 To 0 Step -1
'				If channels(i).Hidden = hidden Then
'					index += 1
'				End If
'				If index = numberBack Then
'					Return channels(i)
'				End If
'			Next
'			Return Nothing
'		End Function
'		Private Sub DrawChannelIfLegalPlacement(apoint As Point, bpoint As Point, cpoint As Point, setPoints As Boolean, dontColorBars As Boolean, hidden As Boolean, isTrendModeChannel As Boolean)
...
'				If ChannelMode <> ChannelModeType.Merged And False Then
'					go = CurrentSwing.ChannelLine Is Nothing
'				Else
'					If channels.Count > 1 AndAlso CurrentChannelLineIfPotential(hidden) IsNot Nothing Then
'						go = CurrentChannelLineIfPotential(hidden).CPoint.X <> cpoint.X
'					Else
'						go = True
'					End If
'				End If
...
'					End If
'				End If
'                If bcExceeded And hidden = False Then Exit Sub
'				Dim isCurrentChannelLinePotential As Boolean = channels.Count > 0 AndAlso Not channels(channels.Count - 1).IsConfirmed
'				If go Then ' if no channel on current swing
'					For Each ch In channels
'						If ch.APoint = apoint And ch.Hidden = False And ch.Swing Is CurrentSwing Then
'                            RemoveObjectFromChart(ch.TL)
'                            RemoveObjectFromChart(ch.MiddleTL)
'							RemoveObjectFromChart(ch.BCLengthText)
'							channels.Remove(ch)
'							Exit For
'						End If
'					Next
'                    Dim channel As New Channel(NewTrendLine(If(MakeOuterLinesTransparent = False, If(swingDir = Direction.Down, PotentialUpChannelLineColor, PotentialDownChannelLineColor), Colors.Transparent),
'                   apoint, CurrentSwing.TL.EndPoint, ChannelLinesVisible And PotentialChannelLinesVisible And Not hidden))
'                    channel.MiddleTL = NewTrendLine(If(swingDir = Direction.Down, PotentialUpChannelLineColor, PotentialDownChannelLineColor), New Point(0, 0), New Point(0, 0), ChannelLinesVisible And PotentialChannelLinesVisible And Not hidden)
'                    If hidden = True Then
'                        Dim d As New Object
'                    End If
...
'                    channel.MiddleTL.ExtendRight = True
...
'					channel.APoint = apoint
'					channel.BPoint = bpoint
'					channel.CPoint = cpoint
...
'					channel.IsTrendModeChannel = isTrendModeChannel
'					If bcExceeded And hidden = False Then channel.DontDraw = True
'					'If bcExceeded Then channel.TL.ExtendLeft = True
'					'If hidden Then channel.TL.ExtendLeft = True
'					BackupChannel(channel)
'				Else
'					' update points
'					If CurrentChannelLineIfPotential(hidden) IsNot Nothing Then
'						CurrentChannelLineIfPotential(hidden).TL.EndPoint = CurrentSwing.TL.EndPoint
'						If setPoints Then
'							CurrentChannelLineIfPotential(hidden).TL.StartPoint = apoint
'							Dim curChannel As Channel = CurrentChannelLineIfPotential(hidden)
'						End If
'					End If
'				End If
'				' update
...
'                        Dim middleLine As LineCoordinates = CalculateNewStartPointRegressionMiddle(apoint.X, apoint, cpoint)
'                        curChannel.MiddleTL.StartPoint = middleLine.StartPoint
'                        curChannel.MiddleTL.EndPoint = New Point(cpoint.X, LinCalc(middleLine.StartPoint.X, middleLine.StartPoint.Y, middleLine.EndPoint.X, middleLine.EndPoint.Y, cpoint.X))
...
'                                curChannel.BCLengthText.Font.FontWeight = LengthTextFontWeight
...
'						CurrentSwing.ChannelLine = CurrentChannelLineIfPotential(hidden)
'						CurrentChannelLineIfPotential(hidden).Swing = CurrentSwing
...
'				End If
'				Return Max(priceDifference / Tan(averageSwingAngle), (CurrentBar.Number + newLineBarSkip) - cs.EndBar)
'			Else
'				Return 0
'			End If
'		End Function
...
'			Else
'				Dim rvBars As Integer = Ceiling(rv / If(CustomRangeValue = -1, Chart.Settings("RangeValue").Value, CustomRangeValue)) - 1
...
'						channel.TargetText.Location = New Point(endBar, channel.DPoint.Y - BooleanToInteger(channel.Direction) * (bcWidth + 1) * If(CustomRangeValue = -1, Chart.Settings("RangeValue").Value, CustomRangeValue))
'					End If
'				Next
'			End If
'		End Sub
...
'			swings = Me.swings
'            If curSwingEvent = AutoTrendBase.SwingEvent.NewSwing Then
...
'                                RemoveObjectFromChart(.MiddleTL)
'                                .TL = NewTrendLine(SwingChannelPotentialColor, swing.TL.StartPoint, cPoint, Not RemoveSwingChannelPotentials)
'                                .MiddleTL = NewTrendLine(SwingChannelMiddlePotentialColor, swing.TL.StartPoint, cPoint, Not RemoveSwingChannelPotentials)
'								.TL.HasParallel = True
'                                .TL.ParallelDistance = .BPoint.Y - LinCalc(.APoint.X, .APoint.Y, .CPoint.X, .CPoint.Y, .BPoint.X)
'                                .TL.ExtendRight = True
'                                .MiddleTL.ExtendRight = True
'								.BCLengthMet = False
'							End With
...
'								.TL.EndPoint = highBar
'                                .TL.ParallelDistance = .BPoint.Y - LinCalc(.APoint.X, .APoint.Y, .CPoint.X, .CPoint.Y, .BPoint.X)
'                                .MiddleTL.StartPoint = New Point(.TL.StartPoint.X, .TL.StartPoint.Y + .TL.ParallelDistance / 2)
'                                .MiddleTL.EndPoint = New Point(.TL.EndPoint.X, .TL.EndPoint.Y + .TL.ParallelDistance / 2)
'							End With
'						End If
'						currentSwingPotentialBCTargetText.Text = GetDollarValue(Abs(swingChannel.BPoint.Y - swingChannel.CPoint.Y))
'						currentSwingPotentialBCTargetText.Location = AddToX(swingChannel.CPoint, -1)
'                        swingChannel.TL.Pen.Thickness = 1
'                        swingChannel.MiddleTL.Pen.Thickness = 1
'                        If UseRegressionPotentialSwingChannels Then
'                            Dim middleLine As LineCoordinates = CalculateNewStartPointRegressionMiddle(swingChannel.TL.StartPoint.X, swingChannel.APoint, swingChannel.CPoint)
'                            Dim cdif As Decimal = swingChannel.CPoint.Y - middleLine.EndPoint.Y
'                            Dim adif As Decimal = swingChannel.APoint.Y - middleLine.StartPoint.Y
'                            Dim dif As Decimal = adif - cdif
'                            swingChannel.TL.StartPoint = New Point(swingChannel.TL.StartPoint.X, swingChannel.APoint.Y - dif)
'                            swingChannel.TL.ParallelDistance = swingChannel.BPoint.Y - LinCalc(swingChannel.TL.StartPoint.X, swingChannel.TL.StartPoint.Y, swingChannel.TL.EndPoint.X, swingChannel.TL.EndPoint.Y, swingChannel.BPoint.X)
'                            swingChannel.MiddleTL.StartPoint = middleLine.StartPoint
'                            swingChannel.MiddleTL.EndPoint = New Point(swingChannel.TL.EndPoint.X, LinCalc(middleLine.StartPoint.X, middleLine.StartPoint.Y, middleLine.EndPoint.X, middleLine.EndPoint.Y, swingChannel.TL.EndPoint.X))
'                        End If
'						If lowBar.Y <= swingChannel.BPoint.Y Then
'							'swingChannel.IsActive = False
'                            swingChannel.TL.Pen.Thickness = 0
'                            swingChannel.MiddleTL.Pen.Thickness = 0
'							currentSwingPotentialBCTargetText.Text = ""
'                            swingChannel.IsConfirmed = True
'                            If RemoveSwingChannelPotentials Then AddObjectToChart(swingChannel.TL)
'                            If RemoveSwingChannelPotentials Then AddObjectToChart(swingChannel.MiddleTL)
'                            swingChannel.TL.Pen = New Pen(New SolidColorBrush(SwingChannelColor), 0)
'                            swingChannel.MiddleTL.Pen = New Pen(New SolidColorBrush(SwingChannelMiddleColor), 0)
...
'                                RemoveObjectFromChart(confirmedSwingChannels(confirmedSwingChannels.Count - 1).MiddleTL)
...
'                            currentSwingBCLengthText.Font.FontSize = LengthTextFontSize
'                            currentSwingBCLengthText.Font.FontWeight = LengthTextFontWeight
...
'                            With confirmedSwingChannels(confirmedSwingChannels.Count - 1)
'                                .MiddleTL = NewTrendLine(SwingChannelMiddleColor, swingChannel.TL.StartPoint, swingChannel.TL.EndPoint, True)
'                                .Direction = Direction.Down
'                                .TL.HasParallel = True
'                                .TL.ParallelDistance = swingChannel.TL.ParallelDistance
'                                .APoint = swingChannel.APoint
'                                .BPoint = swingChannel.BPoint
'                                .CPoint = swingChannel.CPoint
'                                .IsActive = swingChannel.IsActive
'                                .IsConfirmed = swingChannel.IsConfirmed
'                                'Dim zz = Nothing
'                                .TL.ExtendRight = True
'                                .MiddleTL.ExtendRight = True
'                                .Swing = CurrentSwing
'                                .MiddleTL.StartPoint = New Point(.TL.StartPoint.X, .TL.StartPoint.Y + .TL.ParallelDistance / 2)
'                                .MiddleTL.EndPoint = New Point(.TL.EndPoint.X, .TL.EndPoint.Y + .TL.ParallelDistance / 2)
'                                '.BCSwingLine = NewTrendLine(If(TypeOf swingChannel.TL.Pen.Brush Is SolidColorBrush, CType(swingChannel.TL.Pen.Brush, SolidColorBrush).Color, Colors.Pink), swingChannel.BPoint, swingChannel.CPoint)
'                                CurrentSwing.SwingChannel = confirmedSwingChannels(confirmedSwingChannels.Count - 1)
'                            End With
'                        End If
'                    End If
...
'                                .BCLengthMet = True
'                                .IsConfirmed = False
'                                If RemoveSwingChannelPotentials Then RemoveObjectFromChart(swingChannel.TL)
'                                If RemoveSwingChannelPotentials Then RemoveObjectFromChart(swingChannel.MiddleTL)
'                                .TL.Pen = New Pen(New SolidColorBrush(SwingChannelPotentialColor), 1)
'                                .MiddleTL.Pen = New Pen(New SolidColorBrush(SwingChannelMiddlePotentialColor), 1)
'                                .CPoint = highBar
...
'                        '    End If
'                        'End If
'                        'If swings(swings.Count - 1).SwingChannel.BCLengthMet = False And swings(swings.Count - 1).SwingChannel.IsCut = False Then
'                        'End If
'                    End If
'                End If
...
'                '                        swings(swings.Count - 3).SwingChannel.TL.ExtendRight = True
...
'                '                            swings(swings.Count - 3).SwingChannel.IsGapped = True
'                '                            swings(swings.Count - 3).SwingChannel.TL.Pen.Thickness = SwingChannelConfirmedGappedLineWidth
...
'                        currentSwingPotentialBCTargetText.Text = ""
'                    End If
'                    If swings(swings.Count - 1).SwingChannel.IsCut = False And swings(swings.Count - 1).SwingChannel.BCLengthMet And
...
'                            If UseRegressionConfirmedSwingChannels Then
'                                Dim middleLine As LineCoordinates = CalculateNewStartPointRegressionMiddle(swings(swings.Count - 1).SwingChannel.TL.StartPoint.X, swings(swings.Count - 1).SwingChannel.APoint, swings(swings.Count - 1).SwingChannel.CPoint, Min(CurrentBar.Number, CurrentSwing.EndBar))
'                                Dim cdif As Decimal = swings(swings.Count - 1).SwingChannel.CPoint.Y - middleLine.EndPoint.Y
'                                Dim adif As Decimal = swings(swings.Count - 1).SwingChannel.APoint.Y - middleLine.StartPoint.Y
'                                Dim dif As Decimal = adif - cdif
'                                swings(swings.Count - 1).SwingChannel.TL.StartPoint = New Point(swings(swings.Count - 1).SwingChannel.TL.StartPoint.X, swings(swings.Count - 1).SwingChannel.APoint.Y - dif)
'                                swings(swings.Count - 1).SwingChannel.TL.EndPoint = New Point(Min(CurrentBar.Number, CurrentSwing.EndBar), LinCalc(swings(swings.Count - 1).SwingChannel.TL.StartPoint.X, swings(swings.Count - 1).SwingChannel.TL.StartPoint.Y, swings(swings.Count - 1).SwingChannel.CPoint.X, swings(swings.Count - 1).SwingChannel.CPoint.Y, Min(CurrentBar.Number, CurrentSwing.EndBar)))
'                                swings(swings.Count - 1).SwingChannel.TL.ParallelDistance = swings(swings.Count - 1).SwingChannel.BPoint.Y - LinCalc(swings(swings.Count - 1).SwingChannel.TL.StartPoint.X, swings(swings.Count - 1).SwingChannel.TL.StartPoint.Y, swings(swings.Count - 1).SwingChannel.TL.EndPoint.X, swings(swings.Count - 1).SwingChannel.TL.EndPoint.Y, swings(swings.Count - 1).SwingChannel.BPoint.X)
'                                swings(swings.Count - 1).SwingChannel.MiddleTL.StartPoint = middleLine.StartPoint
'                                swings(swings.Count - 1).SwingChannel.MiddleTL.EndPoint = New Point(swings(swings.Count - 1).SwingChannel.TL.EndPoint.X, LinCalc(middleLine.StartPoint.X, middleLine.StartPoint.Y, middleLine.EndPoint.X, middleLine.EndPoint.Y, swings(swings.Count - 1).SwingChannel.TL.EndPoint.X))
'                            Else
'                                swings(swings.Count - 1).SwingChannel.MiddleTL.EndPoint = New Point(Min(CurrentBar.Number, CurrentSwing.EndBar), LinCalc(swings(swings.Count - 1).SwingChannel.MiddleTL.StartPoint.X, swings(swings.Count - 1).SwingChannel.MiddleTL.StartPoint.Y, swings(swings.Count - 1).SwingChannel.MiddleTL.EndPoint.X, swings(swings.Count - 1).SwingChannel.MiddleTL.EndPoint.Y, Min(CurrentBar.Number, CurrentSwing.EndBar)))
'                            End If
'                        Else
...
'                            If UseRegressionConfirmedSwingChannels Then
'                                Dim middleLine As LineCoordinates = CalculateNewStartPointRegressionMiddle(swings(swings.Count - 1).SwingChannel.TL.StartPoint.X, swings(swings.Count - 1).SwingChannel.APoint, swings(swings.Count - 1).SwingChannel.CPoint)
'                                Dim cdif As Decimal = swings(swings.Count - 1).SwingChannel.CPoint.Y - middleLine.EndPoint.Y
'                                Dim adif As Decimal = swings(swings.Count - 1).SwingChannel.APoint.Y - middleLine.StartPoint.Y
'                                Dim dif As Decimal = adif - cdif
'                                swings(swings.Count - 1).SwingChannel.TL.StartPoint = New Point(swings(swings.Count - 1).SwingChannel.TL.StartPoint.X, swings(swings.Count - 1).SwingChannel.APoint.Y - dif)
'                                swings(swings.Count - 1).SwingChannel.TL.EndPoint = New Point(CurrentBar.Number, LinCalc(swings(swings.Count - 1).SwingChannel.TL.StartPoint.X, swings(swings.Count - 1).SwingChannel.TL.StartPoint.Y, swings(swings.Count - 1).SwingChannel.CPoint.X, swings(swings.Count - 1).SwingChannel.CPoint.Y, CurrentBar.Number))
'                                swings(swings.Count - 1).SwingChannel.TL.ParallelDistance = swings(swings.Count - 1).SwingChannel.BPoint.Y - LinCalc(swings(swings.Count - 1).SwingChannel.TL.StartPoint.X, swings(swings.Count - 1).SwingChannel.TL.StartPoint.Y, swings(swings.Count - 1).SwingChannel.TL.EndPoint.X, swings(swings.Count - 1).SwingChannel.TL.EndPoint.Y, swings(swings.Count - 1).SwingChannel.BPoint.X)
'                                swings(swings.Count - 1).SwingChannel.MiddleTL.StartPoint = middleLine.StartPoint
'                                swings(swings.Count - 1).SwingChannel.MiddleTL.EndPoint = New Point(swings(swings.Count - 1).SwingChannel.TL.EndPoint.X, LinCalc(middleLine.StartPoint.X, middleLine.StartPoint.Y, middleLine.EndPoint.X, middleLine.EndPoint.Y, swings(swings.Count - 1).SwingChannel.TL.EndPoint.X))
'                            Else
'                                swings(swings.Count - 1).SwingChannel.MiddleTL.EndPoint = New Point(CurrentBar.Number, LinCalc(swings(swings.Count - 1).SwingChannel.MiddleTL.StartPoint.X, swings(swings.Count - 1).SwingChannel.MiddleTL.StartPoint.Y, swings(swings.Count - 1).SwingChannel.MiddleTL.EndPoint.X, swings(swings.Count - 1).SwingChannel.MiddleTL.EndPoint.Y, CurrentBar.Number))
'                            End If
'                        End If
...
'                        If swings(swings.Count - 1).SwingChannel IsNot Nothing AndAlso swings(swings.Count - 1).SwingChannel.IsConfirmed And UseRegressionConfirmedSwingChannels Then
'                            Dim middleLine As LineCoordinates = CalculateNewStartPointRegressionMiddle(swings(swings.Count - 1).SwingChannel.TL.StartPoint.X, swings(swings.Count - 1).SwingChannel.APoint, swings(swings.Count - 1).SwingChannel.CPoint)
'                            Dim cdif As Decimal = swings(swings.Count - 1).SwingChannel.CPoint.Y - middleLine.EndPoint.Y
'                            Dim adif As Decimal = swings(swings.Count - 1).SwingChannel.APoint.Y - middleLine.StartPoint.Y
'                            Dim dif As Decimal = adif - cdif
...
'                            swings(swings.Count - 1).SwingChannel.MiddleTL.Pen.DashStyle = TrendLineDashStyle
'                            swings(swings.Count - 1).SwingChannel.MiddleTL.ExtendRight = False
'                        End If
'                    End If
...
'                    End If
'                    'Dim bcHeight As Decimal = Abs(swings(swings.Count - 1).SwingChannel.BPoint.Y - swings(swings.Count - 1).SwingChannel.CPoint.Y)
'                    'Dim yPoint As Double = swings(swings.Count - 1).SwingChannel.DPoint.Y - BooleanToInteger(swings(swings.Count - 1).SwingChannel.Direction) * bcHeight
'                    'Dim endBar = CurrentSwing.EndBar + GetProjectedLineBarSkip(bcHeight)
'                    'If (swings(swings.Count - 1).SwingChannel.Direction = Direction.Down And CurrentSwing.EndPrice < LinCalc(swings(swings.Count - 1).SwingChannel.TL.StartPoint.X,
...
'                    '  endBar) - bcHeight) Or
'                    ' (swings(swings.Count - 1).SwingChannel.Direction = Direction.Up And CurrentSwing.EndPrice > LinCalc(swings(swings.Count - 1).SwingChannel.TL.StartPoint.X, swings(swings.Count - 1).SwingChannel.TL.StartPoint.Y, swings(swings.Count - 1).SwingChannel.TL.EndPoint.X, swings(swings.Count - 1).SwingChannel.TL.EndPoint.Y, endBar) + bcHeight) Then
...
'                            RemoveObjectFromChart(swings(swings.Count - 2).SwingChannel.MiddleTL)
...
'                            'Dim zz = Nothing
'                            If swings(swings.Count - 2).SwingChannel.IsGapped = False Then swings(swings.Count - 2).SwingChannel.TL.ExtendRight = True
'                            If swings(swings.Count - 2).SwingChannel.IsGapped = False Then swings(swings.Count - 2).SwingChannel.MiddleTL.ExtendRight = True
...
'                                swings(swings.Count - 2).SwingChannel.MiddleTL.Pen.DashStyle = TrendLineDashStyle
'                                'swings(swings.Count - 2).SwingChannel.TL.Pen.Brush = New SolidColorBrush(If(swings(swings.Count - 2).Direction = Direction.Up, ConfirmedHistoryUpChannelLineColor, ConfirmedHistoryDownChannelLineColor))
'                                'If swings(swings.Count - 2).SwingChannel.BCSwingLine IsNot Nothing Then swings(swings.Count - 2).SwingChannel.BCSwingLine.Pen.Brush = New SolidColorBrush(HistorySwingChannelColor)
'                                If Not ShowSwingChannelHistoryGapLines And ChannelLinesVisible Then
'                                    AddObjectToChart(swings(swings.Count - 2).SwingChannel.TL)
'                                    AddObjectToChart(swings(swings.Count - 2).SwingChannel.MiddleTL)
'                                End If
'                            End If
...
'                            swings(swings.Count - 2).SwingChannel.MiddleTL.Pen.DashStyle = TrendLineDashStyle
'                            If UseRegressionConfirmedSwingChannels Then
'                                Dim middleLine As LineCoordinates = CalculateNewStartPointRegressionMiddle(swings(swings.Count - 2).SwingChannel.TL.StartPoint.X, swings(swings.Count - 2).SwingChannel.APoint, swings(swings.Count - 2).SwingChannel.CPoint)
'                                Dim cdif As Decimal = swings(swings.Count - 2).SwingChannel.CPoint.Y - middleLine.EndPoint.Y
'                                Dim adif As Decimal = swings(swings.Count - 2).SwingChannel.APoint.Y - middleLine.StartPoint.Y
'                                Dim dif As Decimal = adif - cdif
'                                swings(swings.Count - 2).SwingChannel.TL.StartPoint = New Point(swings(swings.Count - 2).SwingChannel.TL.StartPoint.X, swings(swings.Count - 2).SwingChannel.APoint.Y - dif)
'                                swings(swings.Count - 2).SwingChannel.TL.EndPoint = New Point(CurrentBar.Number, LinCalc(swings(swings.Count - 2).SwingChannel.TL.StartPoint.X, swings(swings.Count - 2).SwingChannel.TL.StartPoint.Y, swings(swings.Count - 2).SwingChannel.CPoint.X, swings(swings.Count - 2).SwingChannel.CPoint.Y, CurrentBar.Number))
'                                swings(swings.Count - 2).SwingChannel.TL.ParallelDistance = swings(swings.Count - 2).SwingChannel.BPoint.Y - LinCalc(swings(swings.Count - 2).SwingChannel.TL.StartPoint.X, swings(swings.Count - 2).SwingChannel.TL.StartPoint.Y, swings(swings.Count - 2).SwingChannel.TL.EndPoint.X, swings(swings.Count - 2).SwingChannel.TL.EndPoint.Y, swings(swings.Count - 2).SwingChannel.BPoint.X)
'                                swings(swings.Count - 2).SwingChannel.MiddleTL.StartPoint = middleLine.StartPoint
'                                swings(swings.Count - 2).SwingChannel.MiddleTL.EndPoint = New Point(swings(swings.Count - 2).SwingChannel.TL.EndPoint.X, LinCalc(middleLine.StartPoint.X, middleLine.StartPoint.Y, middleLine.EndPoint.X, middleLine.EndPoint.Y, swings(swings.Count - 2).SwingChannel.TL.EndPoint.X))
'                            Else
'                                swings(swings.Count - 2).SwingChannel.MiddleTL.EndPoint = New Point(CurrentBar.Number, LinCalc(swings(swings.Count - 2).SwingChannel.MiddleTL.StartPoint.X, swings(swings.Count - 2).SwingChannel.MiddleTL.StartPoint.Y, swings(swings.Count - 2).SwingChannel.MiddleTL.EndPoint.X, swings(swings.Count - 2).SwingChannel.MiddleTL.EndPoint.Y, CurrentBar.Number))
'                            End If
'                            If Not ShowSwingChannelHistoryGapLines And ChannelLinesVisible Then
'                                AddObjectToChart(swings(swings.Count - 2).SwingChannel.TL)
'                                AddObjectToChart(swings(swings.Count - 2).SwingChannel.MiddleTL)
'                            End If
'                        End If
'                        If swings(swings.Count - 2).SwingChannel.IsCut And swings(swings.Count - 2).SwingChannel.BCLengthMet Then
'                            swings(swings.Count - 2).SwingChannel.TL.ExtendRight = False
'                            swings(swings.Count - 2).SwingChannel.TL.Pen.DashStyle = TrendLineDashStyle
'                            swings(swings.Count - 2).SwingChannel.MiddleTL.ExtendRight = False
'                            swings(swings.Count - 2).SwingChannel.MiddleTL.Pen.DashStyle = TrendLineDashStyle
'                            'swings(swings.Count - 2).SwingChannel.TL.Pen.Brush = New SolidColorBrush(If(swings(swings.Count - 2).Direction = Direction.Up, ConfirmedHistoryUpChannelLineColor, ConfirmedHistoryDownChannelLineColor))
'                            'If swings(swings.Count - 2).SwingChannel.BCSwingLine IsNot Nothing Then swings(swings.Count - 2).SwingChannel.BCSwingLine.Pen.Brush = New SolidColorBrush(HistorySwingChannelColor)
'                            If Not ShowSwingChannelHistoryGapLines And ChannelLinesVisible Then
...
'                            swings(swings.Count - 2).SwingChannel.TL.EndPoint = New Point(CurrentSwing.EndBar, LinCalc(swings(swings.Count - 2).SwingChannel.TL.StartPoint.X, swings(swings.Count - 2).SwingChannel.TL.StartPoint.Y, swings(swings.Count - 2).SwingChannel.TL.EndPoint.X, swings(swings.Count - 2).SwingChannel.TL.EndPoint.Y, CurrentSwing.EndBar))
'                            swings(swings.Count - 2).SwingChannel.MiddleTL.EndPoint = New Point(CurrentSwing.EndBar, LinCalc(swings(swings.Count - 2).SwingChannel.MiddleTL.StartPoint.X, swings(swings.Count - 2).SwingChannel.MiddleTL.StartPoint.Y, swings(swings.Count - 2).SwingChannel.MiddleTL.EndPoint.X, swings(swings.Count - 2).SwingChannel.MiddleTL.EndPoint.Y, CurrentSwing.EndBar))
...
'                                RemoveObjectFromChart(swings(swings.Count - 2).SwingChannel.MiddleTL)
'                            Else
'                                If ChannelLinesVisible Then AddObjectToChart(swings(swings.Count - 2).SwingChannel.TL)
'                                If ChannelLinesVisible Then AddObjectToChart(swings(swings.Count - 2).SwingChannel.MiddleTL)
'                            End If
'                        End If
...
'                        '    For barI = confirmedSwingChannels(i).TL.StartPoint.X To CurrentBar.Number - 1
'                        '        points.Add(New Point(barI, Chart.bars(barI).Data.Low + (Chart.bars(barI).Data.High - Chart.bars(barI).Data.Low) / 2))
'                        '    Next
'                        '    Dim cdif As Decimal = confirmedSwingChannels(i).CPoint.Y - CalculateRegressionY(points, confirmedSwingChannels(i).CPoint.X)
'                        '    Dim adif As Decimal = confirmedSwingChannels(i).APoint.Y - CalculateRegressionY(points, confirmedSwingChannels(i).APoint.X)
'                        '    Dim dif As Decimal = adif - cdif
'                        '    confirmedSwingChannels(i).TL.StartPoint = New Point(confirmedSwingChannels(i).TL.StartPoint.X, confirmedSwingChannels(i).APoint.Y - dif)
'                        '    confirmedSwingChannels(i).TL.ParallelDistance = confirmedSwingChannels(i).BPoint.Y - LinCalc(confirmedSwingChannels(i).TL.StartPoint.X, confirmedSwingChannels(i).TL.StartPoint.Y, confirmedSwingChannels(i).TL.EndPoint.X, confirmedSwingChannels(i).TL.EndPoint.Y, confirmedSwingChannels(i).BPoint.X)
'                        'End If
'                        confirmedSwingChannels(i).TL.EndPoint = New Point(CurrentBar.Number, LinCalc(confirmedSwingChannels(i).TL.StartPoint.X, confirmedSwingChannels(i).TL.StartPoint.Y, confirmedSwingChannels(i).CPoint.X, confirmedSwingChannels(i).CPoint.Y, CurrentBar.Number))
'                        If UseRegressionConfirmedSwingChannels Then
'                            Dim middleLine As LineCoordinates = CalculateNewStartPointRegressionMiddle(confirmedSwingChannels(i).TL.StartPoint.X, confirmedSwingChannels(i).APoint, confirmedSwingChannels(i).CPoint, Min(CurrentBar.Number, CurrentSwing.EndBar))
'                            'Dim cdif As Decimal = confirmedSwingChannels(i).CPoint.Y - middleLine.EndPoint.Y
'                            'Dim adif As Decimal = confirmedSwingChannels(i).APoint.Y - middleLine.StartPoint.Y
'                            'Dim dif As Decimal = adif - cdif
'                            'confirmedSwingChannels(i).TL.StartPoint = New Point(0, confirmedSwingChannels(i).APoint.Y - dif)
'                            'confirmedSwingChannels(i).TL.ParallelDistance = confirmedSwingChannels(i).BPoint.Y - LinCalc(confirmedSwingChannels(i).TL.StartPoint.X, confirmedSwingChannels(i).TL.StartPoint.Y, confirmedSwingChannels(i).TL.EndPoint.X, confirmedSwingChannels(i).TL.EndPoint.Y, confirmedSwingChannels(i).BPoint.X)
'                            confirmedSwingChannels(i).MiddleTL.StartPoint = middleLine.StartPoint
'                            confirmedSwingChannels(i).MiddleTL.EndPoint = New Point(confirmedSwingChannels(i).TL.EndPoint.X, LinCalc(middleLine.StartPoint.X, middleLine.StartPoint.Y, middleLine.EndPoint.X, middleLine.EndPoint.Y, confirmedSwingChannels(i).TL.EndPoint.X))
'                        End If
'                        confirmedSwingChannels(i).IsCut = True
'                        'confirmedSwingChannels(i).TL.Pen.Brush = New SolidColorBrush(If(confirmedSwingChannels(i).Direction = Direction.Up, ConfirmedHistoryUpChannelLineColor, ConfirmedHistoryDownChannelLineColor))
'                        confirmedSwingChannels(i).TL.Pen.Thickness = 1
'                        confirmedSwingChannels(i).MiddleTL.Pen.Thickness = 1
'                    End If
'                    If confirmedSwingChannels(i).BCLengthMet And confirmedSwingChannels(i).IsCut Then
'                        confirmedSwingChannels(i).TL.Pen.DashStyle = TrendLineDashStyle
'                        confirmedSwingChannels(i).MiddleTL.Pen.DashStyle = TrendLineDashStyle
'                    End If
...
'                            confirmedSwingChannels(i).MiddleTL.ExtendRight = False
'                        Else
...
'            RemoveObjectFromChart(swingChannel.MiddleTL)
...
'                    channel.TL.Pen = New Pen(New SolidColorBrush(If(MakeOuterLinesTransparent = False, If(channel.Direction = Direction.Down, PotentialHistoryDownChannelLineColor, PotentialHistoryUpChannelLineColor), Colors.Transparent)), 1)
'                    channel.TL.ExtendRight = False
'                    channel.MiddleTL.EndPoint = New Point(channel.CutBarNumber, LinCalc(channel.MiddleTL.StartPoint.X, channel.MiddleTL.StartPoint.Y, channel.MiddleTL.EndPoint.X, channel.MiddleTL.EndPoint.Y, channel.CutBarNumber))
'                    channel.MiddleTL.Pen = New Pen(New SolidColorBrush(If(channel.Direction = Direction.Down, PotentialHistoryDownChannelLineColor, PotentialHistoryUpChannelLineColor)), 1)
'                    channel.MiddleTL.ExtendRight = False
...
'            channel.MiddleTL.Pen.DashStyle = TrendLineDashStyle

'            If HistoryChannelLineParallelsVisible = False Or channel.IsConfirmed = False Then
'                channel.TL.HasParallel = False
'            End If
...
'				ShowNextBackedUpChannel(channel.Direction, True)
'			End If
'            channel.TL.Pen = New Pen(New SolidColorBrush(If(MakeOuterLinesTransparent = False, If(channel.Direction = Direction.Down, ConfirmedHistoryDownChannelLineColor, ConfirmedHistoryUpChannelLineColor), Colors.Transparent)), 1)
'			channel.TL.Pen.DashStyle = TrendLineDashStyle
...
'            channel.TL.ExtendRight = False
'            channel.MiddleTL.Pen = New Pen(New SolidColorBrush(If(channel.Direction = Direction.Down, ConfirmedHistoryDownChannelLineColor, ConfirmedHistoryUpChannelLineColor)), 1)
'            channel.MiddleTL.Pen.DashStyle = TrendLineDashStyle
'            channel.MiddleTL.ExtendRight = False
'		End Sub
...
'                    If backup.IsConfirmed = isConfirmed And backup.IsCut = False And backup.IsCancelled = False And backup.IsGapped = False And backup.GappedSwing Is Nothing And backup.Direction = direction And backup.IsBackedUp = False Then
'                        count += 1
'                    End If
...
'					If isConfirmed Then
'                        If ChannelLinesVisible Then AddObjectToChart(c.TL)
'                        If ChannelLinesVisible Then AddObjectToChart(c.MiddleTL)
'                        c.TL.Pen = New Pen(New SolidColorBrush(If(MakeOuterLinesTransparent = False, If(c.Direction = AutoTrendBase.Direction.Down, ConfirmedDownChannelLineColor, ConfirmedUpChannelLineColor), Colors.Transparent)), 1)
'                        c.MiddleTL.Pen = New Pen(New SolidColorBrush(If(c.Direction = AutoTrendBase.Direction.Down, ConfirmedDownChannelLineColor, ConfirmedUpChannelLineColor)), 1)
'					Else
'                        If c.Hidden = False And c.DontDraw = False And ChannelLinesVisible And PotentialChannelLinesVisible Then
'                            AddObjectToChart(c.TL)
'                            AddObjectToChart(c.MiddleTL)
'                            'Dim zz = c.MiddleTL
'                        End If
'					End If
'					c.IsBackedUp = False
...
'                        If otherChannel.IsConfirmed Then otherChannel.MiddleTL.Pen.Thickness = 1
'						If otherChannel.IsConfirmed Then
'                            RemoveObjectFromChart(otherChannel.TL)
'                            RemoveObjectFromChart(otherChannel.MiddleTL)
