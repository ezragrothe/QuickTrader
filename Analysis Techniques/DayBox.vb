...
#End Region
        ...
        Dim prevSpannedDayCount As Integer
        <Input> Public Property MultipleDaySpan As Integer = 1
        <Input> Public Property BeginningDayOfWeek As Integer = 0
        <Input> Public Property AdjustTimesOnChartPad As Boolean = False

        <Input("", "Display")>
        Public Property AscDesColorsOn As Boolean = True
        <Input> Public Property AscendingDayBoxFillColor As Color = Colors.Lime
        <Input> Public Property DescendingDayBoxFillColor As Color = Colors.Red
        <Input> Public Property UseISOSNHNLColors As Boolean = True
        <Input> Public Property ISDayBoxFillColor As Color = Colors.Gray
        <Input> Public Property OSDayBoxFillColor As Color = Colors.Blue
        <Input> Public Property NHDayBoxFillColor As Color = Colors.Green
        <Input> Public Property NLDayBoxFillColor As Color = Colors.Red
        <Input> Public Property LineColor As Color = Colors.LightGray
        <Input> Public Property BoxLineWidth As Double = 1
        <Input> Public Property ExtendEndLineDown As Boolean = True
        Public Property DashStyle As DoubleCollection = (New DoubleCollectionConverter).ConvertFromString("11 6 3 6 3 6")

        <Input("", "Projections")> Public Property HideLastBox As Boolean = True
        ...
        <Input> Public Property HighTextLocation As Decimal = 300
        <Input> Public Property LowTextLocation As Decimal = 600
        ...
        <Input> Public Property RangeTextSpacing As Decimal = 2
        ...
        Public Class Box
            ...
            Public Property DayRangeLeftLine As TrendLine
            Public Property DayRangeTopLine As TrendLine
            Public Property DayRangeBottomLine As TrendLine
            Public Property DayRangeRightLine As TrendLine
        ...
        Protected Overrides Sub Begin()
            ...
            projectionRangeText = NewLabel("", DayRangeTextColor, New Point(0, 0), False)
            ...
            projectionRangeText.Font.FontWeight = FontWeights.Bold
        ...
        Public Property CurrentBox As Box
            Get
        ...
        Private Sub StartBox()
            ...
            With CurrentBox
                ...
                .RectObj.Pen.DashStyle = New DashStyle(DashStyle, 0)
                .RectObj.Pen.Thickness = If(HideLastBox, 0, If(boxes.Count <= NumberOfBoxesToUseAsAverage, BoxLineWidth, 0))
                .RectObj.IsSelectable = False
                .RectObj.IsEditable = False

                .DayRangeBottomLine = NewTrendLine(New Pen(New SolidColorBrush(LineColor), If(HideLastBox, 0, BoxLineWidth)), New Point(CurrentBar.Number, CurrentBar.High), New Point(CurrentBar.Number, CurrentBar.Low))
                .DayRangeBottomLine.Pen.DashStyle = New DashStyle(DashStyle, 0)
                .DayRangeBottomLine.IsSelectable = False
                .DayRangeBottomLine.IsEditable = False
                .DayRangeLeftLine = NewTrendLine(New Pen(New SolidColorBrush(LineColor), If(HideLastBox, 0, BoxLineWidth)), New Point(CurrentBar.Number, CurrentBar.High), New Point(CurrentBar.Number, CurrentBar.Low))
                .DayRangeLeftLine.Pen.DashStyle = New DashStyle(DashStyle, 0)
                .DayRangeLeftLine.IsSelectable = False
                .DayRangeLeftLine.IsEditable = False
                .DayRangeRightLine = NewTrendLine(New Pen(New SolidColorBrush(LineColor), 0), New Point(CurrentBar.Number, CurrentBar.High), New Point(CurrentBar.Number, CurrentBar.Low))
                .DayRangeRightLine.Pen.DashStyle = New DashStyle(DashStyle, 0)
                .DayRangeRightLine.IsSelectable = False
                .DayRangeRightLine.IsEditable = False
                .DayRangeTopLine = NewTrendLine(New Pen(New SolidColorBrush(LineColor), If(HideLastBox, 0, BoxLineWidth)), New Point(CurrentBar.Number, CurrentBar.High), New Point(CurrentBar.Number, CurrentBar.Low))
                .DayRangeTopLine.Pen.DashStyle = New DashStyle(DashStyle, 0)
                .DayRangeTopLine.IsSelectable = False
                .DayRangeTopLine.IsEditable = False
        ...
        End Sub
        Private Sub StopBox()
            If IsBoxActive And CurrentBox.RectObj.Width <> 0 Then
                ...
                With CurrentBox
                    RemoveObjectFromChart(.DayRangeBottomLine)
                    RemoveObjectFromChart(.DayRangeLeftLine)
                    RemoveObjectFromChart(.DayRangeTopLine)
                    If ExtendEndLineDown Then
                        .DayRangeRightLine.ExtendLeft = True
                        .DayRangeRightLine.Pen.Thickness = BoxLineWidth
                        .DayRangeRightLine.Coordinates = New LineCoordinates(CurrentBar.Number, high.Y, CurrentBar.Number, low.Y)
                    Else
                        RemoveObjectFromChart(.DayRangeRightLine)
                    End If

                    .DayRangeBottomLine = Nothing
                    .DayRangeLeftLine = Nothing
                    .DayRangeRightLine = Nothing
                    .DayRangeTopLine = Nothing
                    ...
                    .RectObj.Fill = New SolidColorBrush(If(Not AscDesColorsOn, Colors.Transparent, If(UseISOSNHNLColors, GetISOSFillColor(), If(low.X < high.X, AscendingDayBoxFillColor, DescendingDayBoxFillColor))))
                    If .RectObj.Coordinates.Right - 1 <= Chart.bars.Count - 1 And .RectObj.Coordinates.Right - 1 >= 0 And ShowCloseDot Then
                        .CloseDot.Coordinates = New LineCoordinates(.RectObj.Coordinates.Right, Bars(.RectObj.Coordinates.Right - 1).Close, .RectObj.Coordinates.Right + 1, Bars(.RectObj.Coordinates.Right - 1).Close)
                    Else
                        .CloseDot.Coordinates = New LineCoordinates(0, 0, 0, 0)
                    ...
                    If ShowDayRange And CurrentBar.Number - .RectObj.Location.X > 5 Then
                        Dim rangetextPoint As Point
                        If low.X > high.X Or True Then
                            rangetextPoint = New Point(.RectObj.Location.X, low.Y - If(ShowTimeInfoTexts, Chart.GetRelativeFromRealHeight(HighLowVerticalSpacing), 0) - GetTextOffset(low.X))
                            If HighLowTextsVisible Then rangetextPoint = AddToY(rangetextPoint, -Chart.GetRelativeFromRealHeight(HighLowTextSize))
                            .RangeText.VerticalAlignment = LabelVerticalAlignment.Top
                        Else
                            rangetextPoint = New Point(.RectObj.Location.X, high.Y + If(ShowTimeInfoTexts, Chart.GetRelativeFromRealHeight(HighLowVerticalSpacing), 0) + GetTextOffset(high.X))
                            If HighLowTextsVisible Then rangetextPoint = AddToY(rangetextPoint, Chart.GetRelativeFromRealHeight(HighLowTextSize))
                            .RangeText.VerticalAlignment = LabelVerticalAlignment.Bottom
                        End If
                    ...
                    If HighLowTextsVisible And CurrentBar.Number - .RectObj.Location.X > 12 Then
                        ...
                        .TimeLineLowTimeText.Text = n(0) & n(1) & ":" & n(2) & n(3)  'If(.TimeLineLowTimeText.Location.X > .TimeLineHighTimeText.Location.X, n(0) & n(1) & ":" & n(2) & n(3) & vbNewLine, "")
                        n = FormatNumberLength(Chart.bars(high.X - 1).Data.Date.Hour, 2) & FormatNumberLength(Chart.bars(high.X - 1).Data.Date.Minute, 2)
                        .TimeLineHighTimeText.Text = n(0) & n(1) & ":" & n(2) & n(3) 'If(.TimeLineLowTimeText.Location.X > .TimeLineHighTimeText.Location.X, "", n(0) & n(1) & ":" & n(2) & n(3))
                    End If
                End With
        ...
        Private Function GetTextOffset(bar As Integer) As Decimal
            ...
            '    If TypeOf item.AnalysisTechnique Is MLRTrend1 AndAlso item.AnalysisTechnique.IsEnabled Then
            '        Return Chart.GetRelativeFromRealHeight(CType(item.AnalysisTechnique, MLRTrend1).GetTextOffset(bar))
            '    ElseIf TypeOf item.AnalysisTechnique Is MLRTrend2 AndAlso item.AnalysisTechnique.IsEnabled Then
            '        Return Chart.GetRelativeFromRealHeight(CType(item.AnalysisTechnique, MLRTrend2).GetTextOffset(bar))
            '    ElseIf TypeOf item.AnalysisTechnique Is FanTrend AndAlso item.AnalysisTechnique.IsEnabled Then
            '        Return Chart.GetRelativeFromRealHeight(CType(item.AnalysisTechnique, FanTrend).GetTextOffset(bar))
            '    End If
            'Next
            Return RangeTextSpacing
        End Function
                    ...
                    If Date.TryParseExact(Me.StartTime, "HHmm", Nothing, Globalization.DateTimeStyles.AssumeLocal, New Date) And Date.TryParseExact(Me.EndTime, "HHmm", Nothing, Globalization.DateTimeStyles.AssumeLocal, New Date) Then
                        ...
                        If boxIsStarted Then
                            ...
                            If IsTimeBetween(prevBarDate, barDate, startTime) Then
                                StopBox()
                                boxIsStarted = False
                                If (DisableSundayBox And barDate.DayOfWeek <> DayOfWeek.Sunday) Or Not DisableSundayBox And boxIsStarted = False Then
                                    StartBox()
                        ...
                        Else
                            ...
                            '    boxIsStarted = True
                            'End If
                            If IsTimeBetween(prevBarDate, barDate, startTime) AndAlso ((Not IsTimeBetween(prevBarDate, barDate, endTime) And DisableSundayBox And barDate.DayOfWeek <> DayOfWeek.Sunday) Or Not DisableSundayBox) And boxIsStarted = False Then
                                StartBox()
                                boxStartDate = barDate
                                boxIsStarted = True
                            Else
                        ...
                        If IsBoxActive Then
                            With CurrentBox
                                ...
                                .DayRangeBottomLine.Coordinates = New LineCoordinates(.RectObj.Location.X, low.Y, .RectObj.Location.X + projectedWidth, low.Y)
                                .DayRangeTopLine.Coordinates = New LineCoordinates(.RectObj.Location.X, high.Y, .RectObj.Location.X + projectedWidth, high.Y)
                                .DayRangeLeftLine.Coordinates = New LineCoordinates(.RectObj.Location.X, high.Y, .RectObj.Location.X, low.Y)
                                .DayRangeRightLine.Coordinates = New LineCoordinates(.RectObj.Location.X + projectedWidth, high.Y, .RectObj.Location.X + projectedWidth, low.Y)

                                .RectObj.Fill = If(HideLastBox Or Not AscDesColorsOn, Brushes.Transparent, New SolidColorBrush(If(UseISOSNHNLColors, GetISOSFillColor(), If(low.X < high.X, AscendingDayBoxFillColor, DescendingDayBoxFillColor))))
                                ...
                                If HighLowTextsVisible And CurrentBar.Number - .RectObj.Location.X > 12 Then
                                    ...
                                    .TimeLineLowTimeText.Text = n(0) & n(1) & ":" & n(2) & n(3) 'If(.TimeLineLowTimeText.Location.X > .TimeLineHighTimeText.Location.X, n(0) & n(1) & ":" & n(2) & n(3), "")
                                End If
                ...
                Else
                    ...
                    If boxIsStarted Then
                        If prevBarDate.DayOfYear <> barDate.DayOfYear And spannedDayCount >= MultipleDaySpan And prevSpannedDayCount < MultipleDaySpan Then
                            ...
                            boxStartDate = barDate
                            boxIsStarted = True
                            spannedDayCount = spannedDayCount - MultipleDaySpan
                            prevSpannedDayCount = 0
                        End If
                    Else
                        If prevBarDate.DayOfYear <> barDate.DayOfYear And barDate.DayOfWeek = BeginningDayOfWeek Then
                            StartBox()
                            spannedDayCount = 0
                            prevSpannedDayCount = 0
                            boxStartDate = barDate
                    ...
                    If IsBoxActive Then
                        With CurrentBox
                            ...
                            .DayRangeBottomLine.Coordinates = New LineCoordinates(.RectObj.Location.X, low.Y, CurrentBar.Number, low.Y)
                            .DayRangeTopLine.Coordinates = New LineCoordinates(.RectObj.Location.X, high.Y, CurrentBar.Number, high.Y)
                            .DayRangeLeftLine.Coordinates = New LineCoordinates(.RectObj.Location.X, high.Y, .RectObj.Location.X, low.Y)
                            .DayRangeRightLine.Coordinates = New LineCoordinates(CurrentBar.Number, high.Y, CurrentBar.Number, low.Y)
                            ...
                            If HighLowTextsVisible And CurrentBar.Number - .RectObj.Location.X > 12 Then
                                .TimeLineLowTimeText.Location = New Point(low.X, low.Y - Chart.GetRelativeFromRealHeight(HighLowVerticalSpacing))
                                Dim n = FormatNumberLength(Chart.bars(low.X - 1).Data.Date.Hour, 2) & FormatNumberLength(Chart.bars(low.X - 1).Data.Date.Minute, 2)
                                .TimeLineLowTimeText.Text = n(0) & vbNewLine & n(1) & vbNewLine & n(2) & vbNewLine & n(3) & vbNewLine
                                .TimeLineHighTimeText.Location = New Point(high.X, high.Y + Chart.GetRelativeFromRealHeight(HighLowVerticalSpacing))
                                n = FormatNumberLength(Chart.bars(high.X - 1).Data.Date.Hour, 2) & FormatNumberLength(Chart.bars(high.X - 1).Data.Date.Minute, 2)
                                .TimeLineHighTimeText.Text = n(0) & vbNewLine & n(1) & vbNewLine & n(2) & vbNewLine & n(3) & vbNewLine
                            End If
                            If ShowDayRange And CurrentBar.Number - .RectObj.Location.X > 5 Then
        ...
        End Function
        Private Function GetISOSFillColor() As Color
            If boxes.Count > 1 Then
                If boxes(boxes.Count - 1).RectObj.Y > boxes(boxes.Count - 2).RectObj.Y Then
                    If boxes(boxes.Count - 1).RectObj.Y - boxes(boxes.Count - 1).RectObj.Height < boxes(boxes.Count - 2).RectObj.Y - boxes(boxes.Count - 2).RectObj.Height Then
                        Return OSDayBoxFillColor
                    Else
                        Return NHDayBoxFillColor
                    End If
                Else
                    If boxes(boxes.Count - 1).RectObj.Y - boxes(boxes.Count - 1).RectObj.Height < boxes(boxes.Count - 2).RectObj.Y - boxes(boxes.Count - 2).RectObj.Height Then
                        Return NLDayBoxFillColor
                    Else
                        Return ISDayBoxFillColor
                    End If
                End If
            Else
                Return OSDayBoxFillColor
        ...
        Public Overrides Property Name As String = "DayBox"
        ...
        Private Sub Elapsed()
            Try
                If Chart IsNot Nothing And timeLabel IsNot Nothing And projectionBox IsNot Nothing Then
                    ...
                    '        If CurrentBar.Number - box.RectObj.Location.X > 12 Then
                    '            box.RangeText.Location = New Point(Avg(box.RectObj.Location.X, box.RectObj.Width + box.RectObj.Location.X), -Chart.GetRelativeFromRealY(RangeTextLocation))
                    '            box.RangeText.HorizontalAlignment = LabelHorizontalAlignment.Center
                    '            box.RangeText.VerticalAlignment = LabelVerticalAlignment.Top
                    '        End If
                    '    Next
                    'End If
                    'For Each box In boxes
                    '    If HighLowTextsVisible And CurrentBar.Number - box.RectObj.Location.X > 12 And low.X - 1 > 0 And high.X - 1 > 0 Then
                    '        box.TimeLineLowTimeText.Location = New Point(box.TimeLineLowTimeText.Location.X, -Chart.GetRelativeFromRealY(LowTextLocation))
                    '        box.TimeLineHighTimeText.Location = New Point(box.TimeLineHighTimeText.Location.X, -Chart.GetRelativeFromRealY(HighTextLocation))
                    '        Dim n = FormatNumberLength(Chart.bars(box.TimeLineLowTimeText.Location.X - 1).Data.Date.Hour, 2) & FormatNumberLength(Chart.bars(box.TimeLineLowTimeText.Location.X - 1).Data.Date.Minute, 2)
                    '        box.TimeLineLowTimeText.Text = n(0) & n(1) & ":" & n(2) & n(3)  'If(.TimeLineLowTimeText.Location.X > .TimeLineHighTimeText.Location.X, n(0) & n(1) & ":" & n(2) & n(3) & vbNewLine, "")
                    '        n = FormatNumberLength(Chart.bars(box.TimeLineHighTimeText.Location.X - 1).Data.Date.Hour, 2) & FormatNumberLength(Chart.bars(box.TimeLineHighTimeText.Location.X - 1).Data.Date.Minute, 2)
                    '        box.TimeLineHighTimeText.Text = n(0) & n(1) & ":" & n(2) & n(3) 'If(.TimeLineLowTimeText.Location.X > .TimeLineHighTimeText.Location.X, "", n(0) & n(1) & ":" & n(2) & n(3))
                    '    End If
                    'Next
