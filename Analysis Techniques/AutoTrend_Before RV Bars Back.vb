...
'    Public Class RegressionAutoTrend
...
'                If key = MoveUpMomentumBarsPosition Then
'                    VerticalMomentumBarsPosition += Chart.Bounds.Height / 15
'                    Chart.ReApplyAnalysisTechnique(Me)
'                ElseIf key = MoveDownMomentumBarsPosition Then
'                    VerticalMomentumBarsPosition -= Chart.Bounds.Height / 15
'                    Chart.ReApplyAnalysisTechnique(Me)
'                End If
'            End If
...
'        <Input()> Public Property CurvedLineFormula As CurveFormula
'        Private _regressionLength As Integer = 90
'        <Input()> Public Property Length As Integer
'            Get
...
'            End Set
'        End Property
'        <Input()> Public Property RVBarsBack As Integer = 40
'        <Input()> Public Property RVMultiplier As Decimal = 1
'        <Input()> Public Property RVLineThickness As Decimal = 0.4
'        <Input()> Public Property RVLineColor As Color = Colors.Black
'        <Input()> Public Property CurvedLineMultiplier As Decimal = 1
'        <Input()> Public Property CurvedLineThickness As Decimal = 0.4
'        <Input()> Public Property CurvedLineColor As Color = Colors.Black
...
'        <Input()> Public Property CenterColor As Color = (New ColorConverter).ConvertFrom("#200000FF")
...
'        <Input(, "Other")> Public Property SyncChartsHotKey As Key = Key.LeftCtrl
'        <Input()> Public Property MomentumBarsVisible As Boolean = True
'        <Input()> Public Property NoOfMomentumBars As Integer = 100
'        <Input()> Public Property VerticalMomentumBarsPosition As Double = 0
'        <Input()> Public Property MomentumOffsetLineMultiplier As Decimal = 2
'        <Input()> Public Property MomentumOffsetLineColor As Color = Colors.Black
'        <Input()> Public Property MomentumOffsetLineThickness As Decimal = 1
...
'            Public Property BCLengths As List(Of BCLength)
...
'            timeLabel = NewLabel(Now.ToShortTimeString, SwingCountTextColor, Chart.GetRelativeFromReal(SwingCountTextLocation), False)
...
'            currentSwingLengthTexts.Add(NewLabel(GetDollarValue(Abs(swings(swings.Count - 1).StartPrice - swings(swings.Count - 1).EndPrice)), Colors.Black, New Point(swings(swings.Count - 1).EndBar, swings(swings.Count - 1).EndPrice), True))
...
'            extendTrendTextObject = NewLabel(rvTextPrefix & "Extend Trend", Colors.Black, New Point(0, 0), False)
'            newTrendTextObject = NewLabel(rvTextPrefix & "New Trend", Colors.Black, New Point(0, 0), True)
...
'            extendSwingTextObject = NewLabel(rvTextPrefix & "Extend Swing", RVLineColor, New Point(0, 0), True)
'            newSwingTextObject = NewLabel(rvTextPrefix & "New Swing", RVLineColor, New Point(0, 0), True)
...
'            currentSwingPotentialBCTargetText = NewLabel("", Colors.Black, New Point(0, 0), True)
...
'                momentumBarsRVLines(i).Pen = New Pen(New SolidColorBrush(RVLineColor), RVLineThickness)
'            Next
...
'            momentumBarsPlots(1) = NewPlot(CenterColor)
'            momentumBarsPlots(2) = NewPlot(DownColor)
'            lastMomentumBar = NewTrendLine(CenterColor)
'            For i = 0 To 1
...
'            curSwingEvent = SwingEvent.None
'            If (cs.Direction = Direction.Down And ((CurrentBar.High >= currentRegressionCenterPoint + RVMultiplier * Chart.Settings("RangeValue").Value) Or CurrentBar.High >= cs.StartPrice)) OrElse
'                (cs.Direction = Direction.Up And ((CurrentBar.Low <= currentRegressionCenterPoint - RVMultiplier * Chart.Settings("RangeValue").Value) Or CurrentBar.Low <= cs.StartPrice)) Then
'                'new swing
...
'                    currentSwingLengthTexts.Add(NewLabel(RemovePrefixZero(GetDollarValue(Abs(swings(swings.Count - 1).StartPrice - swings(swings.Count - 1).EndPrice))) & " ", If(swingDir = Direction.Up, UpSwingChannelColor, DownSwingChannelColor), New Point(swings(swings.Count - 1).EndBar - 1, swings(swings.Count - 1).EndPrice), SwingTextsOn))
'                    currentSwingLengthTexts(currentSwingLengthTexts.Count - 1).Font.FontSize = LengthTextFontSize
'                    currentSwingLengthTexts(currentSwingLengthTexts.Count - 1).Font.FontWeight = LengthTextFontWeight
...
'                    currentSwingLengthTexts(currentSwingLengthTexts.Count - 1).IsSelectable = False
'                    currentSwingLengthTexts(currentSwingLengthTexts.Count - 1).IsEditable = False
...
'                        swings(swings.Count - 1).BCLengths = New List(Of BCLength)
'                    End If
'                End If
...
'                currentSwingLengthTexts(currentSwingLengthTexts.Count - 1).Text = RemovePrefixZero(GetDollarValue(Abs(swings(swings.Count - 1).StartPrice - swings(swings.Count - 1).EndPrice))) & " "
...
'                If SwingChannelLengthAndTargetTextsOn And swings(swings.Count - 1).BCLengths IsNot Nothing Then
'                    ' remove all previous bc lengths
'                    Dim bcLengths As New List(Of BCLength)
'                    While swings(swings.Count - 1).BCLengths.Count > 0
'                        With swings(swings.Count - 1).BCLengths
...
'                            blength.TargetText = NewLabel(predictionTextPrefix & RemovePrefixZero(Round(bcLength, Chart.Settings("DecimalPlaces").Value)) & predictionTextSuffix, If(swings(swings.Count - 1).Direction = Direction.Up, ConfirmedDownChannelLineColor, ConfirmedUpChannelLineColor), New Point(CurrentBar.Number, CurrentSwing.EndPrice - BooleanToInteger(swings(swings.Count - 1).Direction) * bcLength))
...
'                        Else
'                            blength.TargetText.Text = predictionTextPrefix & RemovePrefixZero(Round(bcLength, Chart.Settings("DecimalPlaces").Value)) & predictionTextSuffix
'                        End If
'                    Next
'                    swings(swings.Count - 1).BCLengths = bcLengths
'                End If
...
'                    While indx <= swings(swings.Count - 1).BCLengths.Count - 1
'                        Dim blength As BCLength = swings(swings.Count - 1).BCLengths(indx)
...
'                                swings(swings.Count - 1).BCLengths.RemoveAt(indx)
'                                indx -= 1
'                            End If
'                        End If
'                        indx += 1
'                    End While
'                End If
'                If swings.Count > 1 AndAlso swings(swings.Count - 2).BCLengths IsNot Nothing Then
'                    While swings(swings.Count - 2).BCLengths.Count > 0
'                        With swings(swings.Count - 2).BCLengths
'                            RemoveObjectFromChart(.Item(0).BCProjectionLine)
'                            RemoveObjectFromChart(.Item(0).TargetText)
'                            RemoveObjectFromChart(.Item(0).LengthText)
'                            .Item(0).BCProjectionLine = Nothing
...
'                            Dim blength As BCLength = channel.BCLengths(indx)
'                            If blength.TargetText IsNot Nothing Then
'                                If Abs(highestPoint.Y - lowestPoint.Y) >= blength.BCLength Then ' if bc swing length matched
'                                    RemoveObjectFromChart(blength.TargetText)
'                                    RemoveObjectFromChart(blength.BCProjectionLine)
...
'                        lastMomentumBar.Pen.Brush = GetGradientBarBrush(UpColor, CenterColor, point1)
'                    Else
'                        lastMomentumBar.Pen.Brush = GetGradientBarBrush(CenterColor, DownColor, point2)
'                    End If
...
'            With swing
'                .PushCountText = NewLabel(pushNumber, If(swing.Direction = Direction.Up, TrendChannelUpColor, TrendChannelDownColor), AddToY(swing.EndPoint, If(swings(swings.Count - 1).Direction = Direction.Up, 1, -1) * 0.6 * Chart.Settings("RangeValue").Value))
...
'            End With
'        End Sub
'        Private Function CalculateBCPoints(ByRef bcLengths As List(Of BCLength), direction As Direction, startSwingIndex As Integer, endSwingIndex As Integer) As List(Of BCLength)
...
'                If (direction = AutoTrendBase.Direction.Down And swings(i).EndPrice >= rangePoint.Y) Or (direction = AutoTrendBase.Direction.Up And swings(i).EndPrice <= rangePoint.Y) Then rangePoint = New Point(i, swings(i).EndPrice)
'            Next
'            For i = rangePoint.X To startSwingIndex Step -1
'                If (direction = AutoTrendBase.Direction.Down And swings(i).EndPrice < opposedRangePoint.Y) Or (direction = AutoTrendBase.Direction.Up And swings(i).EndPrice > opposedRangePoint.Y) Then opposedRangePoint = New Point(i, swings(i).EndPrice)
'            Next
...
'            bcLengths.Add(New BCLength With {.BCLength = bcLength})
'            Return CalculateBCPoints(bcLengths, direction, rangePoint.X, endSwingIndex)
'        End Function
'        Private Function CalculateBCPointsForSwing(ByRef bcLengths As List(Of BCLength), direction As Direction, startBar As Integer, endBar As Integer) As List(Of BCLength)
...
'                End If : indx += 1
'            End While
'            If bcLength > Chart.Settings("RangeValue").Value Then bcLengths.Add(New BCLength With {.BCLength = bcLength, .LengthTextLocation = rangePoint})
'            Return CalculateBCPointsForSwing(bcLengths, direction, rangePoint.X, endBar)
'        End Function
'        Private Sub AssignRegressionChannelProperties(newChannel As RegressionChannel, Optional aPoint As Point = Nothing, Optional bcLengths As List(Of BCLength) = Nothing, Optional dPoint As Point = Nothing, Optional startSwingIndex As Integer = 0, Optional endSwingIndex As Integer = 0, Optional showTargetText As Boolean = False)
...
'                    Dim bcLength = blength.BCLength
'                    If blength.TargetText Is Nothing Then
'                        blength.TargetText = NewLabel(predictionTextPrefix & RemovePrefixZero(Round(bcLength, Chart.Settings("DecimalPlaces").Value)) & predictionTextSuffix, If(newChannel.Direction = Direction.Up, ConfirmedDownChannelLineColor, ConfirmedUpChannelLineColor), New Point(CurrentBar.Number, CurrentSwing.EndPrice - BooleanToInteger(newChannel.Direction) * bcLength), True)
'                        blength.TargetText.Font.FontSize = TargetTextFontSize
'                        blength.TargetText.Font.FontWeight = TargetTextFontWeight
'                        blength.TargetText.VerticalAlignment = LabelVerticalAlignment.Center
'                        blength.TargetText.HorizontalAlignment = LabelHorizontalAlignment.Left
...
'            newChannel.BCLengths = New List(Of BCLength)
...
'            newChannel.TL.Pen = New Pen(New SolidColorBrush(color), 0)
...
'                Dim point2 = regressionCenterPoints(CurrentBar.Number) - Abs(CurvedLineMultiplier * Chart.Settings("RangeValue").Value)
'                If Chart.bars(CurrentBar.Number - 1).Data.Low > point2 Then
'                    Chart.bars(CurrentBar.Number - 1).Pen.Brush = GetGradientBarBrush(UpColor, CenterColor, point1)
'                Else
'                    Chart.bars(CurrentBar.Number - 1).Pen.Brush = GetGradientBarBrush(CenterColor, DownColor, point2)
'                End If
'                'Dim belowPoint2 = regressionCenterPoints(CurrentBar.Number) - Abs(CurvedLineMultiplier * Chart.Settings("RangeValue").Value)
'                'Dim abovePoint3 = regressionCenterPoints(CurrentBar.Number) + Abs(CurvedLineMultiplier * 2 * Chart.Settings("RangeValue").Value)
'                'Dim belowPoint3 = regressionCenterPoints(CurrentBar.Number) - Abs(CurvedLineMultiplier * 2 * Chart.Settings("RangeValue").Value)
'                'Dim abovePoint4 = regressionCenterPoints(CurrentBar.Number) + Abs(CurvedLineMultiplier * 3 * Chart.Settings("RangeValue").Value)
'                'Dim belowPoint4 = regressionCenterPoints(CurrentBar.Number) - Abs(CurvedLineMultiplier * 3 * Chart.Settings("RangeValue").Value)
'                'Dim abovePoint5 = regressionCenterPoints(CurrentBar.Number) + Abs(CurvedLineMultiplier * 4 * Chart.Settings("RangeValue").Value)
'                'Dim belowPoint5 = regressionCenterPoints(CurrentBar.Number) - Abs(CurvedLineMultiplier * 4 * Chart.Settings("RangeValue").Value)


'                'If Chart.bars(CurrentBar.Number - 1).Data.Low > abovePoint4 Then
'                '    Chart.bars(CurrentBar.Number - 1).Pen.Brush = GetGradientBarBrush(UpColor5, UpColor4, abovePoint5)
'                'ElseIf Chart.bars(CurrentBar.Number - 1).Data.Low > abovePoint3 Then
'                '    Chart.bars(CurrentBar.Number - 1).Pen.Brush = GetGradientBarBrush(UpColor4, UpColor3, abovePoint4)
'                'ElseIf Chart.bars(CurrentBar.Number - 1).Data.Low > abovePoint2 Then
'                '    Chart.bars(CurrentBar.Number - 1).Pen.Brush = GetGradientBarBrush(UpColor3, UpColor2, abovePoint3)
'                'ElseIf Chart.bars(CurrentBar.Number - 1).Data.Low > point1 Then
'                '    Chart.bars(CurrentBar.Number - 1).Pen.Brush = GetGradientBarBrush(UpColor2, UpColor1, abovePoint2)
'                'ElseIf Chart.bars(CurrentBar.Number - 1).Data.High < belowPoint4 Then
'                '    Chart.bars(CurrentBar.Number - 1).Pen.Brush = GetGradientBarBrush(DownColor4, DownColor5, belowPoint5)
'                'ElseIf Chart.bars(CurrentBar.Number - 1).Data.High < belowPoint3 Then
'                '    Chart.bars(CurrentBar.Number - 1).Pen.Brush = GetGradientBarBrush(DownColor3, DownColor4, belowPoint4)
'                'ElseIf Chart.bars(CurrentBar.Number - 1).Data.High < belowPoint2 Then
'                '    Chart.bars(CurrentBar.Number - 1).Pen.Brush = GetGradientBarBrush(DownColor2, DownColor3, belowPoint3)
'                'ElseIf Chart.bars(CurrentBar.Number - 1).Data.High < point1 Then
'                '    Chart.bars(CurrentBar.Number - 1).Pen.Brush = GetGradientBarBrush(DownColor1, DownColor2, belowPoint2)
'                'Else
'                '    Chart.bars(CurrentBar.Number - 1).Pen.Brush = GetGradientBarBrush(UpColor1, DownColor1, point1)
'                'End If
'                RefreshObject(Chart.bars(CurrentBar.Number - 1))
...
'                    endPoint = Min(maxRangePoint, Max(cs.StartPrice, currentRegressionCenterPoint - RVMultiplier * Chart.Settings("RangeValue").Value))
'                Else
'                    endPoint = Max(maxRangePoint, Min(cs.StartPrice, currentRegressionCenterPoint + RVMultiplier * Chart.Settings("RangeValue").Value))
'                End If
'                'Else
...
'                newSwingTextObject.Location = New Point(CurrentBar.Number, rvProNewLine.EndPoint.Y)
...
'                    Next
'                Next
'                If SwingChannelLengthAndTargetTextsOn And CurrentSwing.BCLengths IsNot Nothing Then
'                    With CurrentSwing
'                        For Each blength In CurrentSwing.BCLengths
'                            If blength.TargetText IsNot Nothing Then
'                                Dim bcHeight As Decimal = blength.BCLength
