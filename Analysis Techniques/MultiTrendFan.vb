...
Namespace AnalysisTechniques
    Public Class MultiTrendFan
        ...
        Dim confirmedRegressionLine As TrendLine
        ...
        <Input> Public Property TrendUpFillColor As Color = Colors.Green
        <Input> Public Property TrendDownFillColor As Color = Colors.Red
        <Input> Public Property SubtrendUpFillColor As Color = Colors.Green
        <Input> Public Property SubtrendDownFillColor As Color = Colors.Red
        ...
        Protected Overrides Sub Begin()
            ...
            potentialRegressionLine.Pen = New Pen(New SolidColorBrush(Colors.Gray), PotentialRegressionLineThickness)
            potentialRegressionLine.IsRegressionLine = True
            potentialRegressionLine.ExtendRight = False
            confirmedRegressionLine = NewTrendLine(Colors.Gray, New Point(CurrentBar.Number, CurrentBar.Close), New Point(CurrentBar.Number, CurrentBar.Close))
            confirmedRegressionLine.Pen.Thickness = 0
            confirmedRegressionLine.IsRegressionLine = True
            confirmedRegressionLine.DrawZoneFill = True
            confirmedRegressionLine.NeutralZoneBarStart = Integer.MaxValue
            confirmedRegressionLine.ConfirmedZoneBarStart = 0
            confirmedRegressionLine.UpZoneFillBrush = New SolidColorBrush(UpRegressionLineColor)
            confirmedRegressionLine.DownZoneFillBrush = New SolidColorBrush(DownRegressionLineColor)
        ...
        Private Sub DrawLines()
            ...
            For Each item In bcLines
                RemoveObjectFromChart(item.PotentialTrendLine)
        ...
        Private Sub DrawLines(bar As BarData)
            ...
            Dim addBCLine =
                Sub()
                    ...
                    With bcLines(bcLines.Count - 1)
                        .PotentialTrendLine = NewTrendLine(Colors.Gray, CurrentTrend.StartPoint, CurrentSwing.EndPoint, False)
                        .PotentialTrendLine.IsRegressionLine = True
                        .PotentialTrendLine.Pen.Thickness = PotentialRegressionLineThickness
                        ...
                        .TrendLine.DrawZoneFill = True
                        .TrendLine.NeutralZoneBarStart = Integer.MaxValue
                        .TrendLine.ConfirmedZoneBarStart = 0
                        .TrendLine.UpZoneFillBrush = New SolidColorBrush(TrendUpFillColor)
                        .TrendLine.DownZoneFillBrush = New SolidColorBrush(TrendDownFillColor)
                        .Line = NewTrendLine(TextLineColor, CurrentSwing.EndPoint, CurrentSwing.EndPoint)
                        .Line.Pen.Thickness = PotentialBCLineThickness
                    ...
                    '    bcLines(bcLines.Count - 2).TrendLine.Pen.Brush = New SolidColorBrush(If(currentTrendDir, UpRegressionLineColor, DownRegressionLineColor))
                    'End If
                End Sub
            ...
            Dim aPoint As Point = New Point(0, 0), bPoint As Point = New Point(0, If(swingDir = False, Decimal.MinValue, Decimal.MaxValue)), cPoint As Point = New Point(CurrentSwing.EndBar, CurrentSwing.EndPrice)
            Dim aIndex As Integer = 0
            'find b point
            For i = swings.Count - 2 To 0 Step -1
                If (swingDir = False And swings(i).StartPrice < CurrentSwing.EndPrice) Or (swingDir = True And swings(i).StartPrice > CurrentSwing.EndPrice) Then
                    aIndex = i
                    aPoint = New Point(swings(i).StartBar, swings(i).StartPrice)
                    Exit For
                End If
            Next
            For i = aIndex To swings.Count - 1
                If (swingDir = Direction.Down And swings(i).StartPrice > bPoint.Y) Or
                   (swingDir = Direction.Up And swings(i).StartPrice < bPoint.Y) Then
                    bPoint = New Point(swings(i).StartBar, swings(i).StartPrice)
                End If
            Next
            'find apoint
            For i = aIndex To 0 Step -1
                If (swingDir = False And swings(i).StartPrice < aPoint.Y) Or (swingDir = True And swings(i).StartPrice > aPoint.Y) Then
                    aPoint = New Point(swings(i).StartBar, swings(i).StartPrice)
                ElseIf Abs(swings(i).StartPrice - aPoint.Y) >= Abs(bPoint.Y - cPoint.Y) Then
                    Exit For
                End If
                If i = 0 Then aPoint = CurrentTrend.StartPoint
            Next
            If newSwingEvent Then addBCLine()
            If bcLines.Count > 0 Then
                If bPoint.X <> swings(0).StartBar Then
                    With bcLines(bcLines.Count - 1)
                        .Line.StartPoint = bPoint
                        .Line.EndPoint = cPoint
                        .APoint = aPoint
                        ...
                        .LengthText.VerticalAlignment = If(Not swingDir, LabelVerticalAlignment.Top, LabelVerticalAlignment.Bottom)
                    ...
                    For i = bcLines.Count - 2 To 0 Step -1
                        If bcLines(i).Line.StartPoint = bPoint And bcLines(i).IsHit = False Then
                            bcLines(i).IsHit = True
                            RemoveObjectFromChart(bcLines(i).TargetText)
                            RemoveObjectFromChart(bcLines(i).TrendLine)
                            RemoveObjectFromChart(bcLines(i).PotentialTrendLine)
                            RemoveObjectFromChart(bcLines(i).Line)
                            RemoveObjectFromChart(bcLines(i).LengthText)
                        End If
                    Next
                Else
                    bcLines(bcLines.Count - 1).IsHit = True
                    RemoveObjectFromChart(bcLines(bcLines.Count - 1).TargetText)
                    RemoveObjectFromChart(bcLines(bcLines.Count - 1).Line)
                    RemoveObjectFromChart(bcLines(bcLines.Count - 1).TrendLine)
                    RemoveObjectFromChart(bcLines(bcLines.Count - 1).PotentialTrendLine)
                    RemoveObjectFromChart(bcLines(bcLines.Count - 1).LengthText)
                End If
                'check for hits
                For i = 0 To bcLines.Count - 2
                    If bcLines(i).IsHit = False And Round(Abs(bcLines(bcLines.Count - 1).Line.EndPoint.Y - bcLines(bcLines.Count - 1).Line.StartPoint.Y), 5) >= Round(Abs(bcLines(i).Line.EndPoint.Y - bcLines(i).Line.StartPoint.Y), 5) And
                                                    bcLines(i).Direction = bcLines(bcLines.Count - 1).Direction Then
            ...
            'check confirmed
            For Each item In bcLines
                If item.IsConfirmed = False And item.IsHit = False Then
                    If ((item.Direction = Direction.Down And CurrentSwing.EndPrice >= item.Line.StartPoint.Y) Or
                       (item.Direction = Direction.Up And CurrentSwing.EndPrice <= item.Line.StartPoint.Y)) And swingDir <> item.Direction Then
                        item.IsConfirmed = True
                        item.Line.Pen.Thickness = ConfirmedBCLineThickness
                        item.DPoint = CurrentSwing.EndPoint
                        item.TrendLine.Pen.Brush = New SolidColorBrush(If(item.Line.StartPoint.X < CurrentTrend.EndBar, If(currentTrendDir, UpRegressionLineColor, DownRegressionLineColor), TextLineColor))
                        item.PotentialTrendLine.Pen.Brush = New SolidColorBrush(If(item.Direction, DownPotentialRegressionLineColor, UpPotentialRegressionLineColor))
                        AddObjectToChart(item.TrendLine)
                        AddObjectToChart(item.PotentialTrendLine)
                    End If
                End If
                If item.IsConfirmed And item.IsHit And ((item.Direction = Direction.Down And CurrentSwing.EndPrice >= item.DPoint.Y) Or
                                                         (item.Direction = Direction.Up And CurrentSwing.EndPrice <= item.DPoint.Y) Or
                                                         (item.Direction = Direction.Down And CurrentSwing.EndPrice <= item.APoint.Y) Or
                                                         (item.Direction = Direction.Up And CurrentSwing.EndPrice >= item.APoint.Y)) Then
                    RemoveObjectFromChart(item.TrendLine)
                    RemoveObjectFromChart(item.PotentialTrendLine)
                End If
                If item.IsConfirmed And ((item.Direction = Direction.Down And CurrentSwing.EndPrice >= item.DPoint.Y) Or
                                         (item.Direction = Direction.Up And CurrentSwing.EndPrice <= item.DPoint.Y)) Then
                    If item.APoint.X = CurrentTrend.StartBar Then
                        item.TrendLine.UpZoneFillBrush = Brushes.Transparent 'New SolidColorBrush(If(currentTrendDir, TrendUpFillColor, TrendDownFillColor))
                        item.TrendLine.DownZoneFillBrush = Brushes.Transparent 'New SolidColorBrush(If(currentTrendDir, TrendUpFillColor, TrendDownFillColor))
                        item.TrendLine.Pen.Thickness = 0
                        RemoveObjectFromChart(item.PotentialTrendLine)
                    ElseIf item.DPoint.X <= CurrentTrend.EndBar Then
                        item.TrendLine.UpZoneFillBrush = New SolidColorBrush(If(currentTrendDir, SubtrendUpFillColor, SubtrendDownFillColor))
                        item.TrendLine.DownZoneFillBrush = New SolidColorBrush(If(currentTrendDir, SubtrendUpFillColor, SubtrendDownFillColor))
                        item.TrendLine.Pen.Thickness = ConfirmedRegressionLineThickness
                    Else
                        item.TrendLine.UpZoneFillBrush = New SolidColorBrush(NeutralFillColor)
                        item.TrendLine.DownZoneFillBrush = New SolidColorBrush(NeutralFillColor)
                        item.TrendLine.Pen.Thickness = LastConfirmedRegressionLineThickness
                    End If

                    item.TrendLine.StartPoint = item.APoint
                    item.TrendLine.EndPoint = CurrentSwing.EndPoint
                    item.DPoint = CurrentSwing.EndPoint
            ...
            For Each item In bcLines
                item.PotentialTrendLine.Coordinates = New LineCoordinates(item.APoint, New Point(CurrentBar.Number, CurrentBar.Close))
                If Not item.IsHit And item.IsConfirmed Then
                    If item.Line.StartPoint.Y <> item.Line.EndPoint.Y Then
                        Dim rangePoint As Double = If(item.Direction, Double.MaxValue, 0)
                        For i = item.Line.EndPoint.X - 1 To CurrentBar.Number - 1
                            If item.Direction = Direction.Down Then
                        ...
                        End If
                        item.TargetText.Text = Strings.StrDup(BCTargetTextLineLength, "─") & FormatNumberLengthAndPrefix(Abs(item.Line.StartPoint.Y - item.Line.EndPoint.Y)) ' & " " &
        ...
        Protected Overrides Sub Main()
            ...
            potentialRegressionLine.Pen.Brush = New SolidColorBrush(If(currentTrendDir = False, DownPotentialRegressionLineColor, UpPotentialRegressionLineColor))
            confirmedRegressionLine.UpZoneFillBrush = New SolidColorBrush(If(currentTrendDir, TrendUpFillColor, TrendDownFillColor))
            confirmedRegressionLine.DownZoneFillBrush = New SolidColorBrush(If(currentTrendDir, TrendUpFillColor, TrendDownFillColor))
            confirmedRegressionLine.Coordinates = CurrentTrend.TL.Coordinates

            If CurrentSwing.EndBar <> CurrentTrend.EndBar Then ' if theres been at least one swing since the end of the trend
                'counterPotentialRegressionLine.StartPoint = CurrentTrend.EndPoint
                'counterPotentialRegressionLine.EndPoint = New Point(CurrentBar.Number, CurrentBar.Close)
                'counterPotentialRegressionLine.Pen.Brush = New SolidColorBrush(If(currentTrendDir, DownPotentialRegressionLineColor, UpPotentialRegressionLineColor))
            End If
        ...
        Friend Overrides Sub OnLoaded()
            For i = potentialRegressionLine.StartPoint.X - 1 To potentialRegressionLine.EndPoint.X - 1
                If i < Chart.bars.Count And Chart.bars.Count <> 0 Then
        ...
        Function CreateText(position As Point, direction As Direction, value As String, color As Color) As Label
            ...
            Return lbl
        End Function
        Public Overrides Property Name As String = "MultiTrendFan"
