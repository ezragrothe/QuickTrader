...
'            Description = "Main AutoTrendV2 analysis technique."
'            Name = "AutoTrendV3"
...
'                            If TypeOf a.AnalysisTechnique Is AutoTrendV2 Then
'                                CType(a.AnalysisTechnique, AutoTrendV2).ApplyRV(value)
'                            End If
'                        Next
'                    End If
'                    Chart.ReApplyAnalysisTechnique(Me)
...
'        <Input("The usable range for a slave chart.")>
'        Public Property SlaveChartBaseRV As Decimal = 1
...
'        Public Property ShowGapMarkers As Boolean = True
...
'        <Input("The color for the potential swing channel.")>
...
'        <Input("The option to remove gapped channel line parallels.")>
'        Public Property RemoveGappedChannelLineParallels As Boolean = True
...
'            rvSecondaryText.Font.FontSize = TargetTextFontSize
'            rvSecondaryText.Font.FontWeight = TargetTextFontWeight
...
'            currentSwingPotentialBCTargetText = NewLabel("", SwingChannelPotentialColor, New Point(0, 0), True)
...
'            swingChannel = New SwingChannelLine(NewTrendLine(SwingChannelPotentialColor, New Point(0, 0), New Point(0, 0), True))
...
'                'For Each analysisTechnique In sc.AnalysisTechniques
'                '    sc.ReApplyAnalysisTechnique(analysisTechnique.AnalysisTechnique)
'                'Next
...
'        Private lastLow As Decimal
'        Private lastHigh As Decimal
'        Private lastDirection As Boolean
'        Private lastEndBar As Integer '
...
'                ElseIf HideLastSwingLine AndAlso swings.Count > 0 And SwingLinesVisible Then
'                End If
...
'                curSwingEvent = AutoTrendV2.SwingEvent.NewSwing
...
'                curSwingEvent = AutoTrendV2.SwingEvent.Extension
'                swingDir = Direction.Up
'                If swings.Count > 1 Then
'                    If EnableBarRVSlaveChart Then
'                        Dim sc = barSlaveChart
...
'            If swings.Count > 2 AndAlso curSwingEvent <> AutoTrendV2.SwingEvent.None Then ' AndAlso (channelLines.Count = 0 OrElse swingDir <> CurrentChannelLine.Direction) Then
...
'                Dim highestAngle As Decimal
'                For i = swings.Count - 2 To 0 Step -1
'                    Dim val = Atan((CurrentSwing.EndPrice - swings(i).StartPrice) / (CurrentSwing.EndBar - swings(i).StartBar)) * 180 / PI
'                    If (swingDir = Direction.Down And val > highestAngle) Or (swingDir = Direction.Up And val < highestAngle) Then
...
'                    End If
'                Next
'                If highestAngle <> 0 Then
'                    aPoint = New Point(rangeBar, rangePoint)
'                End If
'                rangeBar = 0
'                rangePoint = If(swingDir = Direction.Down, Decimal.MinValue, Decimal.MaxValue)
'                ' find B-point
'                For i = aIndex To swings.Count - 1
...
'                setPoints = True
'                DrawChannelIfLegalPlacement(aPoint, bPoint, cPoint, setPoints, True, False)
...
'                                channel.TL.Pen = New Pen(New SolidColorBrush(If(channel.Direction = Direction.Down, ConfirmedDownChannelLineColor, ConfirmedUpChannelLineColor)), 1.5)
...
'                                ' target text
'                                channel.TargetText = NewLabel("- " & FormatNumber(bcLength, DecimalPlaces), If(channel.Direction = Direction.Down, ConfirmedDownChannelLineColor, ConfirmedUpChannelLineColor), New Point(CurrentBar.Number, CurrentSwing.EndPrice - BooleanToInteger(channel.Direction) * bcLength), BCTargetTextsVisible)
...
'                            (
'                                (channel.IsConfirmed) Or
'                                (
'                                    channel.IsConfirmed = False And channel.IsCancelled = False
'                                )
'                            ) Then
'                            CutChannel(channel)
'                        End If
'                        If channel.IsConfirmed And channel.BCSwingMatched And channel.IsCut = False And swingDir = channel.Direction And channel.IsGapped = False Then
'                            If Abs(bPoint.Y - cPoint.Y) >= Abs(channel.BPoint.Y - channel.CPoint.Y) Then ' if bc swing length matched
'                                ' gap channel
...
'                                If RemoveGappedChannelLineParallels Then channel.TL.HasParallel = False
'                                If RV < HistoryBCLengthVisibilityRVCutoff * Chart.Settings("RangeValue").Value Then RemoveObjectFromChart(channel.BCLengthText)
'                                ShowNextBackedUpChannel(channel.Direction, True)
'                                If ShowOnlyLastConfirmedChannels Then
'                                    channel.IsBackedUp = False
'                                    channel.TL.Pen.Thickness = 1
'                                End If
'                            End If
'                        End If
'                    End If
...
'            If ColorBars Then
'                If CurrentBar.Low < lastLow Then
'                    If lastDirection = False Then
'                        'BarColorRoutine(lastEndBar, CurrentBar.Number - 1, DownTrendBarColor)
'                        BarColor(lastEndBar, CurrentBar.Number - 1, DownTrendBarColor, DownTrendSwingLineColor, Swing.ChannelDirectionType.Down, , , , False)
'                        lastEndBar = CurrentBar.Number - 1
'                    Else
'                        'BarColorRoutine(1, CurrentBar.Number - 1, DownTrendBarColor)
'                        BarColor(1, CurrentBar.Number - 1, DownTrendBarColor, DownTrendSwingLineColor, Swing.ChannelDirectionType.Down, , , , False)
'                        lastEndBar = CurrentBar.Number - 1
'                    End If
'                    lastDirection = False
'                    lastLow = CurrentBar.Low
'                ElseIf CurrentBar.High > lastHigh Then
'                    If lastDirection = True Then
'                        'BarColorRoutine(lastEndBar, CurrentBar.Number - 1, UpTrendBarColor)
'                        BarColor(lastEndBar, CurrentBar.Number - 1, UpTrendBarColor, UpTrendSwingLineColor, Swing.ChannelDirectionType.Up, , , , False)
'                        lastEndBar = CurrentBar.Number - 1
'                    Else
'                        'BarColorRoutine(1, CurrentBar.Number - 1, UpTrendBarColor)
'                        BarColor(1, CurrentBar.Number - 1, UpTrendBarColor, UpTrendSwingLineColor, Swing.ChannelDirectionType.Up, , , , False)
'                        lastEndBar = CurrentBar.Number - 1
'                    End If
'                    lastDirection = True
'                    lastHigh = CurrentBar.High
'                End If
'            End If
'        End Sub
...
'                            If channel.IsCut Or channel.IsGapped Then
'                                RemoveObjectFromChart(channel.TL)
'                            ElseIf True Then
'                                ' find A-point 
'                                Dim rangeBar As Integer = 0
'                                Dim rangePoint As Decimal = 0
'                                Dim aIndex As Integer
'                                Dim highestAngle As Decimal
'                                Dim startSwingIndex As Integer = 0
'                                For i = 0 To swings.Count - 1
'                                    If swings(i).EndBar > Chart.Bounds.X1 Then
'                                        startSwingIndex = i
'                                        Exit For
'                                    End If
'                                Next
'                                Dim endSwingIndex As Integer = GetChannelEndSwingIndex(channel)
'                                For i = endSwingIndex - 1 To startSwingIndex Step -1
'                                    Dim val = Atan((swings(endSwingIndex).EndPrice - swings(i).EndPrice) / (swings(endSwingIndex).EndBar - swings(i).EndBar)) * (180 / PI)
'                                    If (channel.Direction = Direction.Up And val > highestAngle) Or (channel.Direction = Direction.Down And val < highestAngle) Then
'                                        rangeBar = swings(i).EndBar
'                                        rangePoint = swings(i).EndPrice
'                                        aIndex = i
'                                        highestAngle = val
'                                    End If
'                                Next
'                                If highestAngle <> 0 Then
'                                    channel.APoint = New Point(rangeBar, rangePoint)
'                                    channel.TL.StartPoint = New Point(rangeBar, rangePoint)
'                                Else
'                                    RemoveObjectFromChart(channel.TL)
'                                End If
'                            End If
'                        ElseIf (
'                                (channel.IsCut = False) Or
'                                (channel.IsCut And
'                                    (
...
'                                                          apoint, CurrentSwing.TL.EndPoint, ChannelLinesVisible And Not hidden))
...
'            Return 0
'        End Function
'        Private Function GetChannelEndSwingIndex(ByVal channel As Channel) As Integer
'            For i = 0 To swings.Count - 1
'                If channel.CPoint.X = swings(i).StartBar Then
'                    Return i - 1
'                End If
'            Next
'            Return 0
...
'        '    End If
'        'End Sub
'        Protected Sub BarColor(ByVal startBar As Integer, ByVal endBar As Integer, ByVal color As Color, swingColor As Color, direction As Swing.ChannelDirectionType, Optional onlyColorNeutral As Boolean = False, Optional neutralColor As Color = Nothing, Optional neutralBarColor As Color = Nothing, Optional setSwingDirections As Boolean = True)
'            Dim swingsToColor As New List(Of Swing)
...
'                    Else
'                        swing.TL.Pen.Brush = New SolidColorBrush(swingColor)
'                        If setSwingDirections Then swing.ChannelDirection = direction
'                    End If
'                End If
'            Next
'            If ColorBars Then
...
'            swings = Me.swings
'            If curSwingEvent = AutoTrendV2.SwingEvent.NewSwing Then
...
'                        currentGapMark.Line = NewTrendLine(If(swings(swings.Count - 1).Direction = Direction.Down, DownGapMarkerColor, UpGapMarkerColor), New Point(0, 0), New Point(0, 0), ShowGapMarkers)
...
'                                .BPoint = bPoint
'                                .CPoint = cPoint
'                                .TL = NewTrendLine(SwingChannelPotentialColor, swing.TL.StartPoint, cPoint)
'                                .TL.HasParallel = True
...
'                            swingChannel.TL.Pen = New Pen(New SolidColorBrush(SwingChannelConfirmedColor), 1)
...
'                End If
'                If confirmedSwingChannels.Count > 0 Then
'                    If confirmedSwingChannels(confirmedSwingChannels.Count - 1).Swing IsNot CurrentSwing Then
'                        RemoveObjectFromChart(currentSwingBCLengthText)
'                        currentSwingPotentialBCTargetText.Text = ""
'                    End If
'                    If confirmedSwingChannels(confirmedSwingChannels.Count - 1).IsCut = False And
'                            ((confirmedSwingChannels(confirmedSwingChannels.Count - 1).Direction = Direction.Down And CurrentBar.High >= LinCalc(confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.StartPoint.X, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.StartPoint.Y, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.EndPoint.X, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.EndPoint.Y, CurrentBar.Number)) Or
'                            (confirmedSwingChannels(confirmedSwingChannels.Count - 1).Direction = Direction.Up And CurrentBar.Low <= LinCalc(confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.StartPoint.X, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.StartPoint.Y, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.EndPoint.X, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.EndPoint.Y, CurrentBar.Number))) Then
'                        confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.EndPoint = New Point(CurrentBar.Number, LinCalc(confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.StartPoint.X, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.StartPoint.Y, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.EndPoint.X, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.EndPoint.Y, CurrentBar.Number))
'                        confirmedSwingChannels(confirmedSwingChannels.Count - 1).IsCut = True
'                        If confirmedSwingChannels(confirmedSwingChannels.Count - 1).BCLengthMet Then confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.Pen.DashStyle = TrendLineDashStyle
'                    End If
'                    If ((confirmedSwingChannels(confirmedSwingChannels.Count - 1).Direction = Direction.Down And (CurrentBar.Low <= confirmedSwingChannels(confirmedSwingChannels.Count - 1).DPoint.Y Or confirmedSwingChannels(confirmedSwingChannels.Count - 1).DPoint.Y = 0)) Or
'                        (confirmedSwingChannels(confirmedSwingChannels.Count - 1).Direction = Direction.Up And CurrentBar.High >= confirmedSwingChannels(confirmedSwingChannels.Count - 1).DPoint.Y)) Then
'                        confirmedSwingChannels(confirmedSwingChannels.Count - 1).DPoint = New Point(CurrentBar.Number, If(confirmedSwingChannels(confirmedSwingChannels.Count - 1).Direction = Direction.Down, CurrentBar.Low, CurrentBar.High))
'                        confirmedSwingChannels(confirmedSwingChannels.Count - 1).IsCut = False
'                    End If
'                    'If confirmedSwingChannels(confirmedSwingChannels.Count - 1).IsCut Then
'                    '    confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.Pen.DashStyle = TrendLineDashStyle
'                    'End If
'                    If confirmedSwingChannels(confirmedSwingChannels.Count - 1).BCLengthMet Then
'                        confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.ExtendRight = False
'                        If Not confirmedSwingChannels(confirmedSwingChannels.Count - 1).IsCut Then
'                            confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.EndPoint = New Point(swingChannel.CPoint.X, LinCalc(confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.StartPoint.X, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.StartPoint.Y, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.EndPoint.X, confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.EndPoint.Y, swingChannel.CPoint.X))
'                        Else
'                            confirmedSwingChannels(confirmedSwingChannels.Count - 1).TL.Pen.DashStyle = TrendLineDashStyle
'                        End If
'                    End If
'                    'And confirmedSwingChannels(confirmedSwingChannels.Count - 1).BCLengthMet
'                End If
'                If swings.Count > 1 Then
...
'                            If Not swings(swings.Count - 2).SwingChannel.IsCut Then
...
'            If channel.IsBackedUp = False And channel.IsGapped = False Then
'                ShowNextBackedUpChannel(channel.Direction, True)
'            End If
'            channel.TL.Pen = New Pen(New SolidColorBrush(If(channel.Direction = Direction.Down, ConfirmedHistoryDownChannelLineColor, ConfirmedHistoryUpChannelLineColor)), 1)
'            channel.TL.Pen.DashStyle = TrendLineDashStyle
...
'                    If backup.Direction = direction And backup.IsBackedUp And backup.IsCut = False And backup.IsCancelled = False And backup.IsConfirmed = isConfirmed And backup.IsGapped = False Then
'                        c = channels(i)
'                        Exit For
'                    End If
'                Next
...
'                    If isConfirmed Then
'                        c.TL.Pen = New Pen(New SolidColorBrush(If(c.Direction = AutoTrendV2.Direction.Down, ConfirmedDownChannelLineColor, ConfirmedUpChannelLineColor)), 1)
'                    Else
...
'            Dim shares As Integer = Chart.Settings("DefaultOrderQuantity").Value
'            'Dim a = priceDif / Chart.GetMinTick * Chart.GetMinTick * multiplier * shares
'            Return Round(dollarDif / (multiplier * shares), 8)
'        End Function
