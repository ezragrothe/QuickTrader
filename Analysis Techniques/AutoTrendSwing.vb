...
'                RV2 = bcLengths(bcLengths.Count - 1)
...
'        <Input("The inside swing reversal value.")>
'        Public Property ISRV As Decimal
'        Enum CurveFormula
'            MovingAverage
'            Regression
'        End Enum
...
'        <Input()> Public Property TrendIndicationLineWidth As Decimal = 1
...
'        <Input> Public Property ColorSwingBySwing As Boolean = False
...
'        <Input()> Public Property RVTextFontSize As Double = 15
...
'        <Input()> Public Property SwingChannelNeutralColor As Color
...
'        <Input(, "Swing Lines")>
'        Public Property SwingLineThickness As Decimal = 0.8
...
'        Private isSwings As List(Of Swing)
...
'        Const newRVTextPrefix As String = "── "
...
'        Dim lastSwingPotentialLine As TrendLine
...
'            If Chart IsNot Nothing Then
'                If CustomRangeValue = -1 Then
'                    RV2 = Chart.Settings("RangeValue").Value * DefaultAutoTrendRV
'                Else
'                    RV2 = CustomRangeValue * DefaultAutoTrendRV
'                End If
...
'            isSwings = New List(Of Swing)
'            isSwings.Add(New Swing(NewTrendLine(UpSwingChannelColor,
'                      New Point(CurrentBar.Number, CurrentBar.Close),
'                      New Point(CurrentBar.Number, CurrentBar.Close),
'                      True), Direction.Down))
...
'            lastSwingPotentialLine = NewTrendLine(SwingChannelNeutralColor, SwingChannelOn)
'            lastSwingPotentialLine.IsRegressionLine = True
'            lastSwingPotentialLine.OuterPen = New Pen(New SolidColorBrush(SwingChannelNeutralColor), SwingChannelThickness)
'            lastSwingPotentialLine.Pen = New Pen(New SolidColorBrush(SwingChannelNeutralColor), SwingChannelThickness)
'            If SwingChannelNeutralLineIsDashed Then
'                lastSwingPotentialLine.OuterPen.DashStyle = TrendLineDashStyle
'                lastSwingPotentialLine.Pen.DashStyle = TrendLineDashStyle
'            End If
'            lastSwingPotentialLine.LockToEnd = True
'            lastSwingPotentialLine.HasParallel = True
'            lastSwingPotentialLine.IsSelectable = False
'            lastSwingPotentialLine.IsEditable = False
'            lastSwingPotentialLine.ExtendRight = True

'            currentSwingLengthTexts = New List(Of Label)
'            currentSwingLengthTexts.Add(NewLabel(GetDollarValue(Abs(swings(swings.Count - 1).StartPrice - swings(swings.Count - 1).EndPrice)), Colors.Black, New Point(swings(swings.Count - 1).EndBar, swings(swings.Count - 1).EndPrice), SwingTextsOn, , False))
...
'            extendSwingTextObject = NewLabel(rvTextPrefix & " Ext Swing", RVLineColor, New Point(0, 0), True, , False)
'            newSwingTextObject = NewLabel(newRVTextPrefix & FormatNumberLengthAndPrefix(RV2, Chart.Settings("DecimalPlaces").Value) & " RV ", ColorConverter.ConvertFromString("#FF0067FF"), New Point(0, 0), True, , False)
'            extendSwingTextObject.Font.FontSize = TrendTextFontSize
'            extendSwingTextObject.Font.FontWeight = TrendTextFontWeight
'            newSwingTextObject.Font.FontSize = RVTextFontSize
...
'            barsBackMark.Pen.Thickness = RVLineThickness
...
'        Dim curISSwingEvent As SwingEvent = SwingEvent.None
...
'            'inside swing
'            Dim cs As Swing = isSwings(isSwings.Count - 1)
'            curISSwingEvent = SwingEvent.None
'            If (cs.Direction = Direction.Down And Round(CurrentBar.High, 5) >= Round(cs.EndPoint.Y + ISRV, 5)) OrElse
'                (cs.Direction = Direction.Up And Round(CurrentBar.Low, 5) <= Round(cs.EndPoint.Y - ISRV, 5)) Then

'                isSwings.Add(NewSwing(If(ColorSwingBySwing, If(cs.Direction = Direction.Down, UpTrendSwingLineColor, DownTrendSwingLineColor), If(cs.Direction = Direction.Down, UpSwingChannelColor, DownSwingChannelColor)), New Point(cs.EndBar, cs.EndPrice),
'                                    New Point(CurrentBar.Number, If(cs.Direction = Direction.Down, CurrentBar.High, CurrentBar.Low)), True, Not cs.Direction))

'                isSwings(isSwings.Count - 1).TL.IsSelectable = False
'                isSwings(isSwings.Count - 1).TL.IsEditable = False
'                curISSwingEvent = AutoTrendBase.SwingEvent.NewSwing
'            ElseIf (isSwings.Count > 0 AndAlso CurrentBar.High >= cs.EndPrice AndAlso cs.Direction = Direction.Up) OrElse (isSwings.Count > 0 AndAlso CurrentBar.Low <= cs.EndPrice AndAlso cs.Direction = Direction.Down) Then
'                'extension
...
'            If (cs.Direction = Direction.Down And Round(CurrentBar.High, 5) >= Round(cs.EndPoint.Y + RV2, 5)) OrElse
'                (cs.Direction = Direction.Up And Round(CurrentBar.Low, 5) <= Round(cs.EndPoint.Y - RV2, 5)) Then
'                'new swing
...
'                                    New Point(CurrentBar.Number, If(cs.Direction = Direction.Down, CurrentBar.High, CurrentBar.Low)), False, Not cs.Direction))
'                If swings.Count > 1 And False Then
...
'                currentSwingLengthTexts.Add(NewLabel(FormatNumberLengthAndPrefix(GetDollarValue(Abs(swings(swings.Count - 1).StartPrice - swings(swings.Count - 1).EndPrice)), Chart.Settings("DecimalPlaces").Value) & " ", If(swingDir = Direction.Up, UpSwingChannelColor, DownSwingChannelColor), New Point(swings(swings.Count - 1).EndBar - 1, swings(swings.Count - 1).EndPrice), SwingTextsOn, , True))
'                currentSwingLengthTexts(currentSwingLengthTexts.Count - 1).Font.FontSize = LengthTextFontSize
'                currentSwingLengthTexts(currentSwingLengthTexts.Count - 1).Font.FontWeight = LengthTextFontWeight
...
'                AddHandler currentSwingLengthTexts(currentSwingLengthTexts.Count - 1).MouseDownEvent,
'                    Sub(sender As Object)
'                        ApplyRV(CDec(CType(sender, Label).Text) / 10 ^ Chart.Settings("DecimalPlaces").Value + Chart.GetMinTick)
'                    End Sub
'                swings(swings.Count - 1).LengthText = currentSwingLengthTexts(currentSwingLengthTexts.Count - 1)
...
'                End If
'                lastSwingPotentialLine.StartPoint = CurrentSwing.StartPoint
'                lastSwingPotentialLine.EndPoint = CurrentSwing.EndPoint
'                'End If
...
'                currentSwingLengthTexts(currentSwingLengthTexts.Count - 1).Text = FormatNumberLengthAndPrefix(GetDollarValue(Abs(swings(swings.Count - 1).StartPrice - swings(swings.Count - 1).EndPrice)), Chart.Settings("DecimalPlaces").Value) & " "
...
'            lastSwingPotentialLine.EndPoint = New Point(CurrentBar.Number, CurrentBar.High)
...
'                            blength.LengthText = NewLabel(FormatNumberLengthAndPrefix(Round(bcLength, Chart.Settings("DecimalPlaces").Value), Chart.Settings("DecimalPlaces").Value), If(swings(swings.Count - 1).Direction = Direction.Up, ConfirmedDownChannelLineColor, ConfirmedUpChannelLineColor), AddToX(blength.LengthTextLocation, 1))
...
'                End If
'                If regressionChannels.Count > 0 AndAlso regressionChannels(regressionChannels.Count - 1).EndSwingIndex <= swings.Count - 2 Then ' if it's been at least 1 swing since last confirmed channel
'                    ' update potential line
...
'                            If swings(i).EndPrice <= maxPoint.Y Then maxPoint = New Point(swings(i).EndBar, swings(i).EndPrice)
'                        Else
'                            If swings(i).EndPrice >= maxPoint.Y Then maxPoint = New Point(swings(i).EndBar, swings(i).EndPrice)
'                        End If
'                    Next
...
'                        extendTrendTextObject.Location = New Point(CurrentBar.Number + trendTextOffset, swings(regressionChannels(regressionChannels.Count - 1).EndSwingIndex).EndPrice)
...
'                        extendTrendLineObject.Coordinates = New LineCoordinates(swings(regressionChannels(regressionChannels.Count - 1).EndSwingIndex).EndPoint, New Point(CurrentBar.Number + trendTextOffset, swings(regressionChannels(regressionChannels.Count - 1).EndSwingIndex).EndPrice))
...
'                    End If
'                End If
'                extendSwingTextObject.Text = rvTextPrefix & " Ext Swing"
...
'                    extendSwingTextObject.Text = " Ext Swing & Trend"
'                    extendSwingTextObject.Location = New Point(CurrentBar.Number + trendTextOffset, CurrentSwing.EndPrice)
...
'                    Dim bcLength = blength.BCLength
'                    If blength.TargetText Is Nothing Then
'                        blength.TargetText = NewLabel(predictionTextPrefix & FormatNumberLengthAndPrefix(Round(bcLength, Chart.Settings("DecimalPlaces").Value), Chart.Settings("DecimalPlaces").Value), If(newChannel.Direction = Direction.Up, ConfirmedDownChannelLineColor, ConfirmedUpChannelLineColor), New Point(CurrentBar.Number, CurrentSwing.EndPrice - BooleanToInteger(newChannel.Direction) * bcLength), True, , False)
'                        blength.TargetText.Font.FontSize = TargetTextFontSize
'                        blength.TargetText.Font.FontWeight = TargetTextFontWeight
'                        blength.TargetText.VerticalAlignment = LabelVerticalAlignment.Center
'                        blength.TargetText.HorizontalAlignment = LabelHorizontalAlignment.Left
...
'                    If swings(i).Direction = newChannel.Direction Then
'                        RemoveObjectFromChart(swings(i).LengthText)
'                    End If
'                Next
'            End If
'        End Sub
...
'                    endPoint = CurrentSwing.EndPoint.Y - rv '  Min(maxRangePoint, Max(cs.StartPrice, currentRegressionCenterPoint - RVMultiplier * Chart.Settings("RangeValue").Value))
'                Else
'                    endPoint = CurrentSwing.EndPoint.Y + rv
'                End If
'                'Else
